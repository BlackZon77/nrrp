...
function venderCarro()
	...
	ESX.TriggerServerCallback('esx_carrosolx:requestPlayerCars', function(isOwnedVehicle)
		if isOwnedVehicle then
			...
			ESX.ShowNotification("Este veículo não te pertence!")
		end
	end, vehicleProps.plate)
end
RegisterNetEvent("esx_carrosolx:aceitarproposta")
AddEventHandler("esx_carrosolx:aceitarproposta", function(vendedor,plate,preco)
	local elements = {}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'propsta', {
		...
		if data.current.value == "sim" then
			TriggerServerEvent("esx_carrosolx:respostas",vendedor,true,preco,plate)
		end
		if data.current.value == "nao" then
			TriggerServerEvent("esx_carrosolx:respostas",vendedor,false,preco,plate)
		end
		menu.close()
