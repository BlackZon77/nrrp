			...
			if not HackingATM then
				for k,v in pairs(Config.ATMs) do
					if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, v.x, v.y, v.z, true) <= 1.5 then
						DrawText3Ds(v.x,v.y,v.z, "~g~[G]~s~ para ~r~Roubar~s~ o ~y~Multibanco~s~")
				...
				if policeCount then
					ESX.TriggerServerCallback('esx_atmRobbery:getHackerDevice', function(hackerDevice)
						if hackerDevice then
							...
							exports['nrrp_progbar']:startUI(12000, "A conectar o dispositivo")
...
function hackingEvent(success)
    ...
    if success then
        ...
        ESX.ShowNotification("Tu ~g~Conseguiste~s~ Hackear o ~y~Multibanco~s~")
    else
		ESX.ShowNotification("Tu ~r~falhaste~s~ o hack ao ~y~Multibanco~s~!")
	end
