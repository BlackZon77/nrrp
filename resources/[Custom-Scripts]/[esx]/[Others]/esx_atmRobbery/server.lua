...
ESX.RegisterServerCallback("esx_atmRobbery:isRobbingPossible",function(source,cb)
	...
	if not CheckCooldownTime(GetPlayerIdentifier(source)) then
		...
		TriggerClientEvent("esx:showNotification",source,string.format("Podes roubar novamente em ~b~%s~s~ minutos",waitTimer))
		cb(true)
	end
end)
...
ESX.RegisterServerCallback("esx_atmRobbery:getOnlinePoliceCount",function(source,cb)
	...
	if policeOnline >= Config.RequiredPolice then
		...
		cb(false)
		TriggerClientEvent('esx:showNotification', source, "Não tens policias suficientes em serviço")
	end
end)
...
ESX.RegisterServerCallback("esx_atmRobbery:getHackerDevice",function(source,cb)
	...
	if xPlayer.getInventoryItem("hackerDevice").count >= 1 then
		...
		cb(false)
		TriggerClientEvent('esx:showNotification', source, "Tu precisas de um ~y~Hacking Device~s~ para hackeares o Multibanco!")
	end
end)
...
AddEventHandler("esx_atmRobbery:success",function()
	...
	if Config.EnableDirtyCash then
		...
		TriggerClientEvent("esx:showNotification",source,"Recebeste ~r~$"..reward.. "~s~ dinheiro sujo~s~")
	else
		xPlayer.addMoney(reward)
		TriggerClientEvent("esx:showNotification",source,"Recebeste ~r~$"..reward.. "~s~ $ ~s~")
	end
end)
