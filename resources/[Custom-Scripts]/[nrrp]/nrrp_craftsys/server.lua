...
RegisterServerEvent('nrrp_craftsys:getPlayerInventory')
AddEventHandler('nrrp_craftsys:getPlayerInventory', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer ~= nil then
		TriggerClientEvent('nrrp_craftsys:openMenu', _source, xPlayer.inventory)
	end
end)
...
RegisterServerEvent('nrrp_craftsys:craftItem')
AddEventHandler('nrrp_craftsys:craftItem', function(ingredients)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local item = findRecipe(ingredients)
	if not item then
		TriggerClientEvent('esx:showNotification', _source, 'Nenhum recipiente encontrado com estes materiais')
	else
		if xPlayer ~= nil then
			if hasAllIngredients(xPlayer.inventory, Config.Recipes[item]) then
				...
				TriggerClientEvent('esx:showNotification', _source, '~y~Item Craftado: ~w~' .. itemLabel(item, xPlayer.inventory))
			else
				TriggerClientEvent('esx:showNotification', _source, 'Não tens materiais suficientes')
			end
		end
	end
end)
