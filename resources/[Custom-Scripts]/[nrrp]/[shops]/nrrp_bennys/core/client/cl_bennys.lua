...
local bennyLocationTuner = vector3(-324.04, -132.87, 38.97)  
local bennyLocationImport = vector3(-320.9, -126.65, 38.37)
local bennyLocation = vector3(-318.33, -119.8, 38.4)

local bennyHeading =325
...
AddEventHandler('event:control:bennys', function(useID)
    if not useMechanicJob or (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'mechanic') then 
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            if useID == 1 and not isPlyInBennys then
        ...
        exports['mythic_notify']:SendAlert('inform', 'Não és mecanico')
    end
end)
            ...
            if nearDefault or nearImport or nearTuner then
                ...
                if isNear(plyPos, bennyLocation, 2) then
                    if not isPlyInBennys then
                        Draw3DText(bennyLocation.x, bennyLocation.y, bennyLocation.z + 0.5, "~p~E:~w~ Entrar no Benny's Motorworks", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                ...
                if isNear(plyPos, bennyLocationImport, 2) then
                    if not isPlyInBennys then
                        Draw3DText(bennyLocationImport.x, bennyLocationImport.y, bennyLocationImport.z + 0.5, "~p~E:~w~ Entrar na Import Shop", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                        if IsControlJustPressed(0, 38) then
                ...
                if isNear(plyPos, bennyLocationTuner, 2) then
                    if not isPlyInBennys then
                        Draw3DText(bennyLocationTuner.x, bennyLocationTuner.y, bennyLocationTuner.z + 0.5, "~p~E:~w~ Entrar na Tuner Shop", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                        if IsControlJustPressed(0, 38) then
...
--#[Event Handlers]#--
local myCar = {}
RegisterNetEvent("nrrp_bennys:purchaseSuccessful")
AddEventHandler("nrrp_bennys:purchaseSuccessful", function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('nrrp_bennys:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent("nrrp_bennys:purchaseFailed")
AddEventHandler("nrrp_bennys:purchaseFailed", function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
