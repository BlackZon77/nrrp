$(function(){
    ...
    window.addEventListener('message', function(event){ //event viene compuesto por showmenu y masc (true = hombre, false = mujer)
        ...
        if (item.type == "clotheshop"){
            ...
            $.post('http://nrrp_skinshop/zoom', JSON.stringify("cara"));
            $('#mascaras').hide();
            $('#mascara').hide();
            $('#corbata').hide();
            $('#mochilas').hide();
    ...
    document.onkeydown = function(data){ // document.onkeyup
        if(data.which == 27){
            if (typeOfShop == true) {
                $.post('http://nrrp_skinshop/exit', JSON.stringify("clothes"));
            }
            else{
                $.post('http://nrrp_skinshop/exit', JSON.stringify("accessory"));
            }
            
        }
        if(data.which == 65 || (data.which == 65 && data.repeat)){ //a if
            //lastTime = Date.now();
            $.post('http://nrrp_skinshop/rotate', JSON.stringify("left"));
        }
        if(data.which == 68 || (data.which == 68 && data.repeat)) { //d if
            //lastTime = Date.now();
            $.post('http://nrrp_skinshop/rotate', JSON.stringify("right"));
        }
        if(data.which == 39) { // right
            //alert("Teste")
            $.post('http://nrrp_skinshop/color', JSON.stringify("right"));
        }
        if(data.which == 37) { // left
            //alert("Teste")
            $.post('http://nrrp_skinshop/color', JSON.stringify("left"));
        }
    }
...
function menu(){
    $(".classes").each(function(i, ojb){
        ...
        $(this).click(function(){       
            if (menu == "mascara" || menu == "chapeu" || menu == "oculos" || menu == "orelha"){
                ...
                $.post('http://nrrp_skinshop/zoom', JSON.stringify("zapatos"));
            }
            else if (menu == "gravata" || menu == "blusa" || menu == "jaqueta" || menu == "colete"){
                $.post('http://nrrp_skinshop/zoom', JSON.stringify("tops"));
            }
            else if (menu == "calca"){
                $.post('http://nrrp_skinshop/zoom', JSON.stringify("pants"));
            }
            else{
                $.post('http://nrrp_skinshop/zoom', JSON.stringify("ropa"));
            }
    ...
    $("#confirm").each(function(i, ojb){
        $(this).click(function(){
           // $.post('http://esx_np_skinshop/endDialog', typeOfShop);
            if (typeOfShop == true){
                $.post('http://nrrp_skinshop/endDialog', JSON.stringify("clothes"));
            }      
            else{
                $.post('http://nrrp_skinshop/endDialog', JSON.stringify("accessory"));
            }
        });
    });
    ...
    $(".botao").each(function(i, ojb){
        $(this).click(function(){
            ...
            $.post("http://nrrp_skinshop/update", JSON.stringify([dados, tipo]));
        })
    })
}