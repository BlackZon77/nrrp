...
function OpenShopMenu()
	...
	function(data, menu)
		if data.current.value == 'outfit' then
			...
			ESX.TriggerServerCallback('nrrp_skinshop:getPlayerDressing', function(dressing)
						...
						if data.current.value == 'change_cloth' then
							...
							ESX.TriggerServerCallback('nrrp_skinshop:getPlayerDressing', function(dressing)
								...
								function(data2, menu)
									...
									ESX.TriggerServerCallback('nrrp_skinshop:buyClothes', function(bought)
										if bought then
											--print('comprado debug')
											-----
											TriggerEvent('skinchanger:getSkin', function(skin)

												ESX.TriggerServerCallback('nrrp_skinshop:getPlayerOutfit', function(clothes)
							...
							ESX.TriggerServerCallback('nrrp_skinshop:getPlayerDressing', function(dressing)
								...
								function(data2, menu)
									menu.close()
									TriggerServerEvent('nrrp_skinshop:removeOutfit', data2.current.value)
			...
			TriggerEvent("nrrp_skinshop:toggleMenu", "clotheshop")
		end
		currentAction     = 'shop_menu'
		--inMenu = false
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('nrrp_skinshop:buyClothes', function(bought)
				if bought then
					...
					ESX.TriggerServerCallback('nrrp_skinshop:checkPropertyDataStore', function(foundStore)
						if foundStore then
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'save_dressing', {
								...
								if data2.current.value == 'yes' then
									ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'outfit_name', {
										...
										TriggerEvent('skinchanger:getSkin', function(skin)
											TriggerServerEvent('nrrp_skinshop:saveOutfit', data3.value, skin)
...
RegisterNUICallback("endDialog", function(data)
	--print(data)
	TriggerEvent("nrrp_skinshop:toggleMenu")
...
AddEventHandler('nrrp_skinshop:hasEnteredMarker', function(zone)
	currentAction     = 'shop_menu'
	currentActionMsg  = _U('press_menu')
	currentActionData = {}
end)

AddEventHandler('nrrp_skinshop:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	currentAction = nil
	inMenu = false
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not hasAlreadyEnteredMarker) or (isInMarker and lastZone ~= currentZone) then
			...
			TriggerEvent('nrrp_skinshop:hasEnteredMarker', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('nrrp_skinshop:hasExitedMarker', lastZone)
		end
