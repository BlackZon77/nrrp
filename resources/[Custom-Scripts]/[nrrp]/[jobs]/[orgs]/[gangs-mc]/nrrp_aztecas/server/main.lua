...
ESX.RegisterServerCallback('esx_aztecasjob:retrieveJobVehicles', function(source, cb, type)
...
RegisterNetEvent('esx_aztecasjob:setJobVehicleState')
AddEventHandler('esx_aztecasjob:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles_jobs SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('[esx_aztecasjob_vehshop] [^3WARNING^7] %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)
