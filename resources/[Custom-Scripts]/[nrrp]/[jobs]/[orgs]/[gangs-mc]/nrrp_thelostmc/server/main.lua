...
ESX.RegisterServerCallback('esx_thelostmcjob:retrieveJobVehicles', function(source, cb, type)
...
RegisterNetEvent('esx_thelostmcjob:setJobVehicleState')
AddEventHandler('esx_thelostmcjob:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles_jobs SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('[esx_thelostmcjob_vehshop] [^3WARNING^7] %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)

RegisterNetEvent('esx_thelostmcjob:confiscatePlayerItem')
...
RegisterNetEvent('esx_thelostmcjob:getStockItem')
...
RegisterNetEvent('esx_thelostmcjob:putStockItems')
...
ESX.RegisterServerCallback('esx_thelostmcjob:getOtherPlayerData', function(source, cb, target, notify)
...
RegisterNetEvent('esx_thelostmcjob:spawned')
...
RegisterNetEvent('esx_thelostmcjob:forceBlip')
