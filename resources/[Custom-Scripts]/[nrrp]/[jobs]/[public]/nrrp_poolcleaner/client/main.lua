...
function StopNPCJob(cancel)
	...
	if cancel then
		...
		TriggerServerEvent('nrrp_poolcleaner:GiveItem')
		StartNPCJob()
		Done = true
	end
...
AddEventHandler('nrrp_poolcleaner:hasEnteredMarker', function(zone)
...
AddEventHandler('nrrp_poolcleaner:hasExitedMarker', function(zone)

	if zone == 'Sale' then
		TriggerServerEvent('nrrp_poolcleaner:stopSale')
	end
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if PlayerData.job ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('nrrp_poolcleaner:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('nrrp_poolcleaner:hasExitedMarker', LastZone)
			end
		end
	end
end)
				...
				if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
					...
					if PlayerData.job.name == Config.nameJob then
						...
						if CurrentAction == 'Sale' then
							TriggerServerEvent('nrrp_poolcleaner:startSale')
						end
