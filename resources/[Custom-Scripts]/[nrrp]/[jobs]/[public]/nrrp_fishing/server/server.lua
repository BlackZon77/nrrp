...
ESX.RegisterServerCallback('nrrp_fishing:getItems', function(source, cb)
...
RegisterServerEvent('nrrp_fishing:caught')
AddEventHandler('nrrp_fishing:caught', function()
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('nrrp_fishing:bait')
AddEventHandler('nrrp_fishing:bait', function(item, amount)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterUsableItem('lrod', function(source)
    ...
    if baitquantity > 0 or ultrabaitquantity > 0 or extremebaitquantity > 0 then
        TriggerClientEvent('nrrp_fishing:boatFishing', source)
    else
        TriggerClientEvent('esx:showNotification', source, _U('no_equipment'))
    end
end)

RegisterServerEvent('nrrp_fishing:sell')
AddEventHandler('nrrp_fishing:sell', function()
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
	...
	if itemAmount > 0 then
		...
		TriggerClientEvent('nrrp_fishing:notify', _source, _U('sold') .. price .. ':-')
	end
end)

RegisterServerEvent('nrrp_fishing:buyEquipment')
AddEventHandler('nrrp_fishing:buyEquipment', function(item, price, amount)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	...
	if(xPlayer.getMoney() >= price) then
		if (item == 'lrod' and itemAmount > 0) or (item == 'lbait' and itemAmount >= 200) or (item == 'lUbait' and itemAmount >= 200) or (item == 'lEbait' and itemAmount >= 200) then
			TriggerClientEvent('nrrp_fishing:notify', _source, _U('too_much'))
		else
			xPlayer.removeMoney(price)
			xPlayer.addInventoryItem(item, amount)
			TriggerClientEvent('nrrp_fishing:notify', _source, 'Compraste ' .. amount .. ' de ' .. ESX.GetItemLabel(item) .. ' por ~g~' .. price.. '€')
		end
	else
		TriggerClientEvent('nrrp_fishing:notify', _source, _U('not_enough'))
	end
end)
