	...
	if CurrentStatus ~= Status.DELIVERY_INACTIVE then
		...
		if CurrentStatus == Status.PLAYER_REMOVED_GOODS_FROM_VEHICLE then
			...
			if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, DeliveryLocation.Item2.x, DeliveryLocation.Item2.y, DeliveryLocation.Item2.z, true) < 1.5 then
				...
				TriggerServerEvent("nrrp_deliveries:finishDelivery:server", CurrentType)
				PlaySound(-1, "Menu_Accept", "Phone_SoundSet_Default", false, 0, true)
...
function StartDelivery(deliveryType)
	TriggerServerEvent("nrrp_deliveries:removeSafeMoney:server", deliveryType)
end
...
function FinishDelivery(deliveryType, safeReturn)
	...
	TriggerServerEvent("nrrp_deliveries:returnSafe:server", deliveryType, safeReturn)
...
Citizen.CreateThread(function()
	...
	TriggerServerEvent("nrrp_deliveries:getPlayerJob:server")
end)
...
RegisterNetEvent('nrrp_deliveries:setPlayerJob:client')
RegisterNetEvent('nrrp_deliveries:startJob:client')
AddEventHandler('esx:setJob', function(job)
	PlayerJob = job.name
end)

AddEventHandler('nrrp_deliveries:setPlayerJob:client', function(job)
	--print("Player job: " .. job)
	PlayerJob = job
end)

AddEventHandler('nrrp_deliveries:startJob:client', function(deliveryType)
