...
RegisterNetEvent('nrrp_lumberjack:recibodatacliente')
AddEventHandler('nrrp_lumberjack:recibodatacliente',function(data)
    maderas = data
end)

local job = nil
AddEventHandler('playerSpawned', function(spawn)
  TriggerServerEvent('nrrp_lumberjack:getJob')
end)
...
TriggerServerEvent('nrrp_lumberjack:getJob')
RegisterNetEvent('nrrp_lumberjack:setJob')
AddEventHandler('nrrp_lumberjack:setJob',function(jobu)
  job = jobu
end)
...
function AbrirMenu()
	...
	ESX.UI.Menu.Open(
		...
		{
			title  = 'Queres que te compre Madeira?',
			align    = 'left',
			elements = elements
		},
		function(data, menu)	
			if data.current.value == 'yes' then
				TriggerServerEvent('nrrp_lumberjack:quitomad')
			end
			menu.close()
		end,
		function(data, menu)
		...
		for i=1, #maderas, 1 do
            if GetDistanceBetweenCoords(coords.x,coords.y,coords.z,maderas[i].x,maderas[i].y,maderas[i].z) < 75 then
                if maderas[i].vida >= 50 then
    		      DrawText3D(maderas[i].x,maderas[i].y,maderas[i].z, "Madeira ~b~"..maderas[i].tipo.."~w~ : ~g~"..maderas[i].vida.."/"..maderas[i].max)
                elseif maderas[i].vida >= 25 then
                   DrawText3D(maderas[i].x,maderas[i].y,maderas[i].z, "Madeira ~b~"..maderas[i].tipo.."~w~ : ~b~"..maderas[i].vida.."/"..maderas[i].max)
                elseif maderas[i].vida < 25 and maderas[i].vida ~= 0 then
                     DrawText3D(maderas[i].x,maderas[i].y,maderas[i].z, "Madeira ~b~"..maderas[i].tipo.."~w~ : ~y~"..maderas[i].vida.."/"..maderas[i].max)
                elseif maderas[i].vida <= 0 then
                     DrawText3D(maderas[i].x,maderas[i].y,maderas[i].z, "Madeira ~b~"..maderas[i].tipo.."~w~ : ~r~ "..maderas[i].vida.."/"..maderas[i].max)  
                end
            end
		end
...
-- Los clicks habrán que equilibrarlos a la dinámica del servidor
    if maderas[madera].vida > 0 then
        if clicks >= 13 then 
            ...
            TriggerServerEvent('nrrp_lumberjack:doymadera',maderas[madera].data)
            TriggerServerEvent('nrrp_lumberjack:recibodata',maderas)
            madera = nil
        else
            clicks = clicks + 1 
            madera = nil
...
local blips = {
    {title="Lenhador", colour=17, id=85, x = 345.94, y = 6496.72, z = 28.92-1},
    {title="Venda de Madeira", colour=17, id=85, x = -552.38,y = 5327.27,z = 73.6},
}
