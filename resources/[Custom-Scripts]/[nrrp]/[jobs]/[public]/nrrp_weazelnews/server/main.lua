...
TriggerEvent('esx_society:registerSociety', 'weazelnews', 'Weasel News', 'society_weazelnews', 'society_weazelnews', 'society_weazelnews', {type = 'private'})
...
AddEventHandler('SyncWerbung', function(inputText)
	if os.time(os.date("!*t")) > (lastAdDate + 1800) then
		...
		TriggerClientEvent("notifications", -1, "#fc8c03", "Justiça", "Isto foi um teste")
	else
		TriggerClientEvent('FehlerWerbung', source, "Os anúncios estão a exibidos nos últimos 30 minutos.")
	end
end)   