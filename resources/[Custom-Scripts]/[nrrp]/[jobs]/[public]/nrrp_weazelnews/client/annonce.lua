...
AddEventHandler("FehlerWerbung", function(inputText)
	...
	SetNotificationMessage("CHAR_ARTHUR", "CHAR_ARTHUR", true, 1, "~w~Weazel ~r~News:~w~", "~r~Ocurreu um erro~s~", "");
			...
			if input == true then
				...
				elseif UpdateOnscreenKeyboard() == 1 then
					...
					if string.len(inputText) > 0 then
						...
						DisplayOnscreenKeyboard(false, "FMMC_KEY_TIP8", "", "Max. 150 letras", "", "", "", 150)
					end
...
AddEventHandler('DisplayWerbung',function(inputText)
	SetNotificationTextEntry("STRING");
	AddTextComponentString(inputText);
	SetNotificationMessage("CHAR_ARTHUR", "CHAR_ARTHUR", true, 1, "~w~Weazel ~r~News:~w~", "Um ~g~anúncio foi publicado!~w~", "");
	DrawNotification(false, true);
end)
