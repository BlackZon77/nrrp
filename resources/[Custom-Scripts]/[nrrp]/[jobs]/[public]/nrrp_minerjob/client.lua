--en la zona de minas: x rocas con un máximo extraíble. Se regenera con el tiempo, si llega a 0 no puedes nrrp_minerjob.
local clicks = 0
local roca = nil
...
RegisterNetEvent('nrrp_minerjob:setMineLevel')
AddEventHandler('nrrp_minerjob:setMineLevel',function(totalLevel)
    level = totalLevel
end)

RegisterNetEvent('nrrp_minerjob:recibodatacliente')
AddEventHandler('nrrp_minerjob:recibodatacliente',function(data)
    rocas = data
end)

local job = nil
AddEventHandler('playerSpawned', function(spawn)
  TriggerServerEvent('nrrp_minerjob:getJob')
end)
...
TriggerServerEvent('nrrp_minerjob:getJob')
RegisterNetEvent('nrrp_minerjob:setJob')
AddEventHandler('nrrp_minerjob:setJob',function(jobu)
  job = jobu
end)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			if data.current.value == 'yes' then
				TriggerServerEvent('nrrp_minerjob:quitomin')
			end
			menu.close()
		end,
		function(data, menu)
    ...
    if roca ~= nil then
        ...
        if rocas[roca].vida > 0 then
           if clicks >= 1 then
                ...
                TriggerServerEvent('nrrp_minerjob:doymineral',rocas[roca].data)
                TriggerServerEvent('nrrp_minerjob:recibodata',rocas)
                roca = nil
            else
                Wait(2500)
                clicks = clicks + 1
