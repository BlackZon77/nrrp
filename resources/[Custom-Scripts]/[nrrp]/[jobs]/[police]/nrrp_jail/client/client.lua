...
AddEventHandler("esx:playerLoaded", function(newData)
	...
	ESX.TriggerServerCallback("nnrp_jail:retrieveJailTime", function(inJail, newJailTime)
		if inJail then
...
RegisterNetEvent("nnrp_jail:openJailMenu")
AddEventHandler("nnrp_jail:openJailMenu", function()
	OpenJailMenu()
end)

RegisterNetEvent("nnrp_jail:jailPlayer")
AddEventHandler("nnrp_jail:jailPlayer", function(newJailTime)
	jailTime = newJailTime
...
RegisterNetEvent("nnrp_jail:unJailPlayer")
AddEventHandler("nnrp_jail:unJailPlayer", function()
	jailTime = 0
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			TriggerServerEvent("nnrp_jail:updateJailTime", jailTime)

			if jailTime == 0 then
				UnJail()

				TriggerServerEvent("nnrp_jail:updateJailTime", 0)
			end
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					TriggerServerEvent("nnrp_jail:prisonWorkReward")
				end
			end
		end
...
function OpenJailMenu()
	...
	function(data, menu)
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	...
								  	TriggerServerEvent("nnrp_jail:jailPlayer", GetPlayerServerId(closestPlayer), jailTime, reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("nnrp_jail:retrieveJailedPlayers", function(playerArray)
				...
				function(data2, menu2)
					...
					TriggerServerEvent("nnrp_jail:unJailPlayer", action)
