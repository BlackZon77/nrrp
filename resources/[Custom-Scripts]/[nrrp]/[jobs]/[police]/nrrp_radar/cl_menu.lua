Citizen.CreateThread(function()	
    WarMenu.CreateMenu('csrRadar', 'Radar')
	WarMenu.CreateSubMenu('alprOptions', 'csrRadar', 'Matriculas')
	WarMenu.CreateSubMenu('radarOptions', 'csrRadar', 'Opções de Radar')
    while true do
        if WarMenu.IsMenuOpened('csrRadar') then
            if WarMenu.Button('~g~~h~Ativar Radar') then
				toggleALPR()
        ...
        elseif WarMenu.IsMenuOpened('alprOptions') then
            ...
            elseif WarMenu.MenuButton('←←← Voltar', 'csrRadar') then
            end			
			
			
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('radarOptions') then
            if WarMenu.Button('Selecionar Velocidade') then
				Radar_SetLimit()
			elseif WarMenu.Button('Reset | Antena Frontal') then
				ResetFrontAntenna()
			elseif WarMenu.Button('Reset | Antena Traseira') then
				ResetRearAntenna()
            elseif WarMenu.MenuButton('←←← Voltar', 'csrRadar') then
            end

            WarMenu.Display()
        elseif IsControlJustReleased(1, 178) and GetLastInputMethod( 0 ) and ( GetVehicleClass(GetVehiclePedIsIn(GetPlayerPed(-1), false)) == 18 )  then --M by default
...
function toggleALPR()
	...
	if DoesEntityExist(GetVehiclePedIsIn(GetPlayerPed(-1))) then
		if ( toggled == true ) and ( class == 18 ) then
			...
			drawNotification("~b~O radar foi desligado!")
			toggled = false
			Wait(20)
			alpr_toggled = false
			radar_toggled = false
		elseif ( toggled == false ) and ( class == 18 ) then
			PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
			drawNotification("~b~O radar foi ligado!")
			toggled = true
			Wait(20)
			alpr_toggled = true
			radar_toggled = true
            ...
            if GetPedInVehicleSeat(vehicle, -1) == l_ped or GetPedInVehicleSeat(vehicle, 0) == l_ped and not IsEntityDead(l_ped) and vehclass == 18 then
                ...
                if (DoesEntityExist(front_radar_veh) and IsEntityAVehicle(front_radar_veh)) then
                    ...
                    elseif (front_radar_plate == " FIVE M ") then
						isFrontStolen = "~r~CARRO ROUBADO ~o~- Cuidado!"
					elseif (front_radar_plate ~= " FIVE M ") then
						isFrontStolen = "~g~N/A"
					end
                ...
                if (DoesEntityExist(rear_radar_veh) and IsEntityAVehicle(rear_radar_veh)) then
                    ...
                    elseif (rear_radar_plate == " FIVE M ") then
						isRearStolen = "~r~CARRO ROUBADO ~o~- Cuidado!"
					elseif (rear_radar_plate ~= " FIVE M ") then
						isRearStolen = "~g~N/A"
					end
				...
				if (alpr_toggled) then
					...
					drawText("E", 1433.0, 889.0+13.0*2, 0.23, false, 0, 235, 235, 235, 230)
					drawText("T", 1433.0, 889.0+13.0*3, 0.23, false, 0, 235, 235, 235, 230)
					drawText("E", 1434.0, 889.0+13.0*4, 0.23, false, 0, 235, 235, 235, 230)
					drawText("L~n~O~n~C~n~K", 1450.0, 891.0, 0.23, false, 0, 0, 0, 0, 255)
				
					if (fplate == "") then
						drawText("MODELO: ~n~COR: ~n~ACUSAÇÕES:", 1620.0+14.0, 885.0, 0.40, false, 4, 235, 235, 235, 230)
					...
					elseif (alpr_lock ~= nil) and (fplate ~= "") then
						drawText(string.format("MODELO: %s~n~COR: %s~n~ACUSAÇÕES: %s", alpr_lock.lock_fmodel, alpr_lock.lock_fcolor, isFrontStolen), 1620.0+14.0, 885.0, 0.40, false, 4, 235, 235, 235, 230)
						...
						drawText(string.format("MODELO: %s~n~COR: %s~n~ACUSAÇÕES: %s", fmodel, color1, isFrontStolen), 1620.0+14.0, 885.0, 0.40, false, 4, 235, 235, 235, 230)
					...
					--REAR
					drawText("T", 1433.0, 986.0, 0.23, false, 0, 235, 235, 235, 230)
					drawText("R", 1433.0, 986.0+13.0, 0.23, false, 0, 235, 235, 235, 230)
					drawText("A", 1433.0, 986.0+13.0*2, 0.23, false, 0, 235, 235, 235, 230)
					drawText("S", 1433.0, 986.0+13.0*3, 0.23, false, 0, 235, 235, 235, 230)
					drawText("E", 1433.0, 986.0+13.0*4, 0.23, false, 0, 235, 235, 235, 230)
					drawText("I", 1433.0, 986.0+13.0*5, 0.23, false, 0, 235, 235, 235, 230)
					drawText("R", 1433.0, 986.0+13.0*6, 0.23, false, 0, 235, 235, 235, 230)
					drawText("E", 1433.0, 986.0+13.0*7, 0.23, false, 0, 235, 235, 235, 230)
					drawText("L~n~O~n~C~n~K", 1450.0, 983.0, 0.23, false, 0, 0, 0, 0, 255)
					if (rplate == "") then
						drawText("MODELO: ~n~COR: ~n~ACUSAÇÕES:", 1620.0+14.0, 999.0-21.0, 0.40, false, 4, 235, 235, 235, 230)
					...
					elseif (alpr_lock ~= nil) and (fplate ~= "") then
						drawText(string.format("MODELO: %s~n~COR: %s~n~ACUSAÇÕES: %s", alpr_lock.lock_rmodel, alpr_lock.lock_rcolor, isRearStolen), 1620.0+14.0, 999.0-21.0, 0.40, false, 4, 235, 235, 235, 230)
						...
						drawText(string.format("MODELO: %s~n~COR: %s~n~ACUSAÇÕES: %s", rmodel, color2, isRearStolen), 1620.0+14.0, 999.0-21.0, 0.40, false, 4, 235, 235, 235, 230)
...
AddEventHandler( 'wk:toggleRadar', function()
    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            Notify( "~r~Tens de estar dentro de um carro da polícia." )
        end 
    else 
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        Notify( "~b~Velocidade mudada para Km/h." )
    else 
...
function ToggleLockBeep()
    if ( radarInfo.lockBeep ) then 
        ...
        Notify( "~b~Aviso de Velocidade desativado." )
    else 
        radarInfo.lockBeep = true
        Notify( "~b~Aviso de Velocidade ativado." )
    end    
end 
