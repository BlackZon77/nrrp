...
AddEventHandler("mdt:getOffenderDetails", function(offender)
	...
	local result = MySQL.Sync.fetchAll('SELECT * FROM `mdt_user` WHERE `char_id` = @id', {
		['@id'] = offender.id
	})
	offender.notes = ""
	...
	local convictions = MySQL.Sync.fetchAll('SELECT * FROM `mdt_convictions` WHERE `char_id` = @id', {
		['@id'] = offender.id
	})
					...
					MySQL.Async.fetchAll('SELECT * FROM `users` WHERE `identifier` = @identifier', {
						...
						if result[1].id == char_id then
							if jailtime and jailtime > 0 then
								...
								TriggerEvent("nnrp_jail:jailPlayer", src, jailtime, jailmsg)
							end
							if fine > 0 then
								TriggerClientEvent("mdt:billPlayer", usource, src, 'society_police', 'Multa: '..jailmsg, fine)
							end
							return
						end
					end)
...
AddEventHandler("mdt:saveOffenderChanges", function(id, changes, identifier)
	local usource = source
	MySQL.Async.fetchAll('SELECT * FROM `mdt_user` WHERE `char_id` = @id', {
		['@id']  = id
	}, function(result)
		if result[1] then
			MySQL.Async.execute('UPDATE `mdt_user` SET `notes` = @notes, `mugshot_url` = @mugshot_url, `bail` = @bail WHERE `char_id` = @id', {
				['@id'] = id,
				['@notes'] = changes.notes,
			...
			MySQL.Async.insert('INSERT INTO `mdt_user` (`char_id`, `notes`, `mugshot_url`, `bail`) VALUES (@id, @notes, @mugshot_url, @bail)', {
				['@id'] = id,
				['@notes'] = changes.notes,
				['@mugshot_url'] = changes.mugshot_url,
				['@bail'] = changes.bail
		...
		if changes.convictions ~= nil then
			for conviction, amount in pairs(changes.convictions) do	
				MySQL.Async.execute('UPDATE `mdt_convictions` SET `count` = @count WHERE `char_id` = @id AND `offense` = @offense', {
					['@id'] = id,
					['@count'] = amount,
					['@offense'] = conviction
				})
		...
		for i = 1, #changes.convictions_removed do
			MySQL.Async.execute('DELETE FROM `mdt_convictions` WHERE `char_id` = @id AND `offense` = @offense', {
				['@id'] = id,
...
AddEventHandler("mdt:submitNewReport", function(data)
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`, `jailtime`) VALUES (@id, @title, @incident, @charges, @author, @name, @date, @jailtime)', {
		['@id']  = data.char_id,
		['@title'] = data.title,
		['@incident'] = data.incident,
		['@charges'] = charges,
	...
	for offense, count in pairs(data.charges) do
		MySQL.Async.fetchAll('SELECT * FROM `mdt_convictions` WHERE `offense` = @offense AND `char_id` = @id', {
			['@offense'] = offense,
			['@id'] = data.char_id
		}, function(result)
			if result[1] then
				MySQL.Async.execute('UPDATE `mdt_convictions` SET `count` = @count WHERE `offense` = @offense AND `char_id` = @id', {
					['@id']  = data.char_id,
					['@offense'] = offense,
					['@count'] = count + 1
				})
			else
				MySQL.Async.insert('INSERT INTO `mdt_convictions` (`char_id`, `offense`, `count`) VALUES (@id, @offense, @count)', {
					['@id']  = data.char_id,
					['@offense'] = offense,
					['@count'] = count
				})
...
AddEventHandler("mdt:getVehicle", function(vehicle)
	...
	local data = MySQL.Sync.fetchAll('SELECT * FROM `mdt_vehicle` WHERE `plate` = @plate', {
		['@plate'] = vehicle.plate
	})
	if data[1] then
	...
	local bail = MySQL.Sync.fetchAll('SELECT `bail` FROM mdt_user WHERE `char_id` = @id', {
		['@id'] = vehicle.owner_id
	})
...
AddEventHandler("mdt:newCall", function(details, caller, coords)
	...
	for i= 1, #xPlayers do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent("mythic_notify:client:SendAlert", source, {type="infom", text="Recebeste uma nova chamada.", length=5000, style = { ['background-color'] = '#ffffff', ['color'] = '#000000' }})
		end
	end
end)
...
AddEventHandler("mdt:saveVehicleChanges", function(data)
	...
	local usource = source
	MySQL.Async.fetchAll('SELECT * FROM `mdt_vehicle` WHERE `plate` = @plate', {
		['@plate'] = plate
	}, function(result)
		if result[1] then
			MySQL.Async.execute('UPDATE `mdt_vehicle` SET `stolen` = @stolen, `notes` = @notes WHERE `plate` = @plate', {
				['@plate'] = data.plate,
				['@stolen'] = data.stolen,
				['@notes'] = data.notes
			})
		else
			MySQL.Async.insert('INSERT INTO `mdt_vehicle` (`plate`, `stolen`, `notes`) VALUES (@plate, @stolen, @notes)', {
				['@plate'] = data.plate,
				['@stolen'] = data.stolen,
				['@notes'] = data.notes
			})
