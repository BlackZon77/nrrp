...
Citizen.CreateThread(function()
    while true do
        ...
        if not isVisible and IsPedInAnyPoliceVehicle(playerPed) and IsControlJustPressed(0, 212) and GetEntitySpeed(playerVeh) < 5.0 then
        ...
        elseif IsControlJustPressed(0, 212) and not IsPedInAnyPoliceVehicle(playerPed) then
            TriggerServerEvent("mdt:hotKeyOpen")
        end
...
RegisterNUICallback("attachToCall", function(data, cb)
    ...
    if not callBlip then
        ...
        AddTextComponentString("Chamada 112")
...
AddEventHandler("mdt:returnOffenderDetails", function(data)
    for i = 1, #data.vehicles do
        data.vehicles[i].model = CarModelToLabel(data.vehicles[i].model)
    end
    SendNUIMessage({
...
AddEventHandler("mdt:returnVehicleDetails", function(data)
    if type(data.model) == 'number' then
        data.model = CarModelToLabel(data.model)
    end
    SendNUIMessage({
