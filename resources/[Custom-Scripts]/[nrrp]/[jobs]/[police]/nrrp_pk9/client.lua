...
Citizen.CreateThread(function()
    ...
    if Config.Command then
        RegisterCommand(Config.Command, function()
            TriggerEvent('nrrp_pk9:openMenu')
        end)
    end
    while true do
        local sleep = 250
...
RegisterNetEvent('nrrp_pk9:openMenu')
AddEventHandler('nrrp_pk9:openMenu', function()
    mainMenu()
end)

RegisterNetEvent('nrrp_pk9:hasDrugs')
AddEventHandler('nrrp_pk9:hasDrugs', function(hadIt)
    if hadIt then
        exports['mythic_notify']:SendAlert('inform', 'Drogas encontrdas')
        loadDict('missfra0_chop_find')
        TaskPlayAnim(policeDog, 'missfra0_chop_find', 'chop_bark_at_ballas', 8.0, -8, -1, 0, 0, false, false, false)
    else
        exports['mythic_notify']:SendAlert('inform', 'Nenhuma droga encontrada')
    end
end)
...
mainMenu = function()
    if PlayerData.job.name == Config.Job then
            ...
            function(data, menu)
                if data.current.value == 'take_out_remove' then
                    if not DoesEntityExist(policeDog) then
                        ...
                        exports['mythic_notify']:SendAlert('inform', 'K9 Spawnado')
                    else
                        exports['mythic_notify']:SendAlert('inform', 'K9 Removido')
                ...
                elseif data.current.value == 'get_in_out' then
                    if not IsPedInAnyVehicle(policeDog, false) then
                        if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(policeDog)) <= 10.0 then
                            ...
                            exports['mythic_notify']:SendAlert('inform', 'O K9 Está muito longe')
                        end
                    else
                ...
                elseif data.current.value == 'attack_closest' then
                    if DoesEntityExist(policeDog) then
                        if not IsPedDeadOrDying(policeDog) then
                            ...
                            exports['mythic_notify']:SendAlert('inform', 'O teu K9 Morreu')
                        end
                    else
                        exports['mythic_notify']:SendAlert('inform', 'Não tens nenhum K9')
                    end
                            ...
                            if GetDistanceBetweenCoords(GetEntityCoords(policeDog), GetEntityCoords(PlayerPedId()), true) <= 3.0 then
                                ...
                                if distance ~= -1 then
                                    if distance <= 3.0 then
                                        local playerPed = GetPlayerPed(player)
                                        if not IsPedInAnyVehicle(playerPed, true) then
                                            TriggerServerEvent('nrrp_pk9:hasClosestDrugs', GetPlayerServerId(player))
                                        end
                                    end
                                end
                            end
        ...
        exports['mythic_notify']:SendAlert('error', 'Não és Polícia')
    end
end
