				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						if data2.current.value == 'check' then
							TriggerServerEvent('nrrp_idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()))
							menu2.close()
						elseif data2.current.value == 'show' then
							local player, distance = ESX.Game.GetClosestPlayer()

							if distance ~= -1 and distance <= 3.0 then
								TriggerServerEvent('nrrp_idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player))
								menu2.close()
							else
...
RegisterNetEvent('nrrp_idcard:open')
AddEventHandler('nrrp_idcard:open', function(playerData)
	cardOpen = true
	SendNUIMessage({
		action = "open",
		array = playerData
