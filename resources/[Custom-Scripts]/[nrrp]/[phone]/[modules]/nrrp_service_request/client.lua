...
nrrp_calls.NetEvent('nrrp_service_request:call',
    function(id)
        while current do
            Citizen.Wait(100)
        end
        ...
        while current do
            ...
            if IsControlJustPressed(0, Config.ControlCode) then
                current = nil
                TriggerServerEvent('nrrp_service_request:accept', id)
                break
            end
...
nrrp_calls.NetEvent('nrrp_service_request:take',
    function(pos, name)
        if blip then
            RemoveBlip(blip)
            blip = nil
...
nrrp_calls.NetEvent('nrrp_service_request:stop',
    function(id)
        if current == id then
            current = nil
        end
