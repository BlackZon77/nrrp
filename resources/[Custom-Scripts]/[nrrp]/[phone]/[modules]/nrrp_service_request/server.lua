nrrp_calls.ESX.Summon(function(ESX)

local calls = {}
local index = 1

nrrp_calls.NetEvent('esx_addons_gcphone:startCall',
	function(source, number, message, coords)
		...
		for _, src in ipairs(ESX.GetPlayers()) do
			if src ~= source then
				...
				if player and player.job and player.job.name == number then
					TriggerClientEvent('nrrp_service_request:call', player.source, id)
					found = true
				end
			end
		end
		...
		TriggerClientEvent('nrrp_service_request:stop', -1, id)
		TriggerClientEvent('esx:showNotification', source, _U('reject'))
	end)

nrrp_calls.NetEvent('nrrp_service_request:accept',
	function(source, id)
		local data = calls[id]
		...
		if data then
			...
			TriggerClientEvent('nrrp_service_request:take', source, data.coords, name)
