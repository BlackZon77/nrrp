...
AddEventHandler("StreetRaces:joinRace_sv", function(index)
    ...
    if playerMoney >= amount then
        ...
        local msg = "Não tens dinheiro suficiente"
            ...
            if race.finishTime == 0 then
                ...
                for _, pSource in pairs(players) do
                    if pSource == source then
                        local msg = ("Ganhaste com um tempo de [%02d:%06.3f]"):format(timeMinutes, timeSeconds)
                        notifyPlayer(pSource, msg)
                    elseif config_sv.notifyOfWinner then
                        local msg = ("%s ganhou com um tempo de [%02d:%06.3f]"):format(getName(source), timeMinutes, timeSeconds)
                        notifyPlayer(pSource, msg)
                    end
                end
            else
                -- Loser, send notification to only the player
                local msg = ("Perdeste com um tempo de [%02d:%06.3f]"):format(timeMinutes, timeSeconds)
                notifyPlayer(source, msg)
            end
...
AddEventHandler("StreetRaces:deleteRace_sv", function(name)
    ...
    if playerRaces[name] ~= nil then
        ...
        -- Send notification to player
        local msg = "Apagada " .. name
        notifyPlayer(source, msg)
    else
        local msg = "Nenhuma corrida encontrada com o nome: " .. name
        notifyPlayer(source, msg)
    end
end)
...
AddEventHandler("StreetRaces:loadRace_sv", function(name)
    ...
    if race ~= nil then
        ...
        -- Send notification to player
        local msg = "Corrida " .. name .. " carregada"
        notifyPlayer(source, msg)
    else
        local msg = "Nenhuma corrida com o nome " .. name .. " foi encontrada"
        notifyPlayer(source, msg)
    end
end)
