    ...
    if gameId ~= nil and betAmount ~= nil and chairId ~= nil then 
        ...
        if not blackjackGameInProgress[gameId] then
            if tonumber(betAmount) then
                ...
                if betAmount > 0 and xPlayer.getMoney() >= betAmount then
                    if betAmount < 75000 then
                        ...
                        TriggerClientEvent("blackjack:notify",source,"~r~Só podes apostar menos de 75.000 €!")
                    end
                end
            end
        end
...
AddEventHandler("Blackjack:standBlackjack",function(gameId,nextCardCount)
    ...
    if blackjackGameData[gameId][source][2][nextCardCount] ~= nil then
