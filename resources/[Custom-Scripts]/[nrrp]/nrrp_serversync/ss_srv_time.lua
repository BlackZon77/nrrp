...
RegisterServerEvent("nrrp_serversync:freezeTime" )
AddEventHandler("nrrp_serversync:freezeTime", function()
	frozen = not frozen
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerClientEvent("nrrp_serversync:updateFromServerTime",-1,secondOfDay,date,frozen)
...
RegisterServerEvent("nrrp_serversync:addTimeChatSuggests" )
AddEventHandler("nrrp_serversync:addTimeChatSuggests", function()
    if IsPlayerAceAllowed(source, "changeTime") then
        TriggerClientEvent("nrrp_serversync:addTimeChatSuggests",source)
    end
end)
...
RegisterCommand('freezetime', function(source,args,rawCommand)
    ...
    if source == 0 then
        TriggerEvent("nrrp_serversync:freezeTime")
