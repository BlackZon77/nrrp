if ss_enable_door_window_sync then
  ...
  RegisterNetEvent('nrrp_serversync:VehicleWindow')
  AddEventHandler( "nrrp_serversync:VehicleWindow", function( playerID, windowsDown )
        ...
        if pressedUp or pressedDown then
          ...
          if GetPedInVehicleSeat( vehicle, - 1 ) == GetPlayerPed( -1 ) then
            if pressedUp then
              ...
              TriggerServerEvent( "nrrp_serversync:SetVehicleWindow", vehicleState.windowsDown )
            end
            if pressedDown then
