if ss_enable_turn_signal_sync then
  ...
  RegisterNetEvent('nrrp_serversync:VehicleIndicator')
  AddEventHandler( "nrrp_serversync:VehicleIndicator", function( playerID, dir, state )
        ...
        if pressedLeft or pressedRight then
          ...
          if GetPedInVehicleSeat( vehicle, - 1 ) == GetPlayerPed( -1 ) then
            if pressedLeft then
              ...
              TriggerServerEvent( "nrrp_serversync:SetVehicleIndicator", 1, vehicleState.indicator.left )
            end
            if pressedRight then
              vehicleState.indicator.right = not vehicleState.indicator.right
              TriggerServerEvent( "nrrp_serversync:SetVehicleIndicator", 0, vehicleState.indicator.right )
            end
          end
        end
      end
