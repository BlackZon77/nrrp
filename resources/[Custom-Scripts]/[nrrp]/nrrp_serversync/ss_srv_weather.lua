...
RegisterServerEvent("nrrp_serversync:changeWeather" )
AddEventHandler("nrrp_serversync:changeWeather", function(startup)
	if startup then
		TriggerClientEvent('nrrp_serversync:changeWeather',source,currentWeather, blackout, startup )
	else
		TriggerClientEvent('nrrp_serversync:changeWeather',-1,currentWeather, blackout, startup )
	end
end)
...
AddEventHandler( "addWeatherChatSuggests", function()
    if IsPlayerAceAllowed(source, "changeWeather") then
        TriggerClientEvent("nrrp_serversync:addWeatherChatSuggests",source)
    end
end)

Citizen.CreateThread(function()
	TraceMsg("^7initialized.")
    while true do
...
function PushNextWeather()
	...
	TriggerEvent("nrrp_serversync:changeWeather",false)
end
        ...
        if IsPlayerAceAllowed(source, "changeWeather") then
            ...
            if args[1] == nil then
                ...
                if validWeatherType then
                    ...
                    weatherTimer = ss_weather_timer * 60
                    if args[2] == "1" then
                        TriggerEvent('nrrp_serversync:changeWeather',true)
                    else
                        TriggerEvent("nrrp_serversync:changeWeather",false)
