...
Citizen.CreateThread(function()
	...
	while true do
		...
		if distance < 1.5 then
			...
			if golfplaying then
				DisplayHelpText("~g~~INPUT_CONTEXT~~s~: Para acabar o golf.")
			else
				DisplayHelpText("~g~~INPUT_CONTEXT~~s~. Para começar a jogar.")
			end
			if (IsControlJustReleased(1, 38)) then
				...
				if (golfplaying) then
					...
					TriggerEvent("customNotification","E: Para lançar, A-D para rodar, H para mudar de taco.")
				end
			end
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == 0 then
			...
			if distance < 50.0 then
				DisplayHelpText("Move a tua bola, ~g~E~s~: para largar a bola se estás preso.")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
...
function endShot()
	...
	if golfstrokes > 12 then
		TriggerEvent("customNotification","Deste demasiado strokes..")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("~g~E~s~: Para largar aqui.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			DisplayHelpText("~g~E~s~: Largar - ~r~ Estás demasiado longe da bola ou perto do buraco.")
		end
	end
end
