...
RegisterCommand("fazerrefem",function(source, args)
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= nil then
			...
			holdingHostage = true 
			TriggerServerEvent('nrrp_hostage:sync', closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag)
		else
...
RegisterNetEvent('nrrp_hostage:syncTarget')
AddEventHandler('nrrp_hostage:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
...
RegisterNetEvent('nrrp_hostage:syncMe')
AddEventHandler('nrrp_hostage:syncMe', function(animationLib, animation,length,controlFlag,animFlag)
	local playerPed = GetPlayerPed(-1)
	ClearPedSecondaryTask(GetPlayerPed(-1))
	RequestAnimDict(animationLib)
	while not HasAnimDictLoaded(animationLib) do
...
RegisterNetEvent('nrrp_hostage:cl_stop')
AddEventHandler('nrrp_hostage:cl_stop', function()
	holdingHostageInProgress = false
	beingHeldHostage = false 
	...
	while true do 
		if holdingHostage then
			if GetEntityHealth(GetPlayerPed(-1)) <= 102 then --You may need to edit this death check for your server
				...
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
				TriggerServerEvent("nrrp_hostage:stop",target)
				Wait(100)
				releaseHostage()
			end 
