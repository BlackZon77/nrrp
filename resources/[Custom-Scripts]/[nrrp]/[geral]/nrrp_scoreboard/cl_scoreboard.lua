...
Citizen.CreateThread(function()
    local function DrawMain()
        ...
        if WarMenu.MenuButton("Disconnects Recentes »", "recent", {r = 0, g = 0, b = 0, a = 150}) then
        end
    end
    ...
    local function DrawOptions()
        if group ~= "user" then
            if WarMenu.Button("Nome:", ST._Scoreboard.SelectedPlayer.name) then end
        end
    ...
    local function Init()
        WarMenu.CreateMenu("scoreboard", "Lista de Jogadores »")
        WarMenu.SetSubTitle("scoreboard", "Jogadores")
        ...
        WarMenu.CreateSubMenu("recent", "scoreboard", "D/C's Recentes")
        ...
        WarMenu.CreateSubMenu("options", "scoreboard", "Info do User")
...
RegisterNetEvent("nrrp_scoreboard:RemovePlayer")
AddEventHandler("nrrp_scoreboard:RemovePlayer", function(data)
    ST.Scoreboard:RemovePlayer(data)
end)

RegisterNetEvent("nrrp_scoreboard:AddPlayer")
AddEventHandler("nrrp_scoreboard:AddPlayer", function(data)
    ST.Scoreboard:AddPlayer(data)
end)

RegisterNetEvent("nrrp_scoreboard:RemoveRecent")
AddEventHandler("nrrp_scoreboard:RemoveRecent", function(src)
    ST.Scoreboard:RemoveRecent(src)
end)

RegisterNetEvent("nrrp_scoreboard:AddAllPlayers")
AddEventHandler("nrrp_scoreboard:AddAllPlayers", function(data, recentData)
