...
AddEventHandler('jobonline:set', function(jobs_online)
	...
	if jobs['mechanic'] > 0 and Config.IfMecaIsOnline == true then
		IsMecanoOnline = true
	else 
...
AddEventHandler('esx_repairkit:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		local vehicle = nil
			exports['mythic_notify']:SendAlert('inform', _U('not_near_engine'))
		else
		...
		then
			...
			Citizen.CreateThread(function()
				...
				exports['nrrp_progbar']:startUI(Config.RepairTime * 1000, _U('ReparingEngine'))
				else
				end
				...
				if explosionchance == 1 and Config.ExplosionOnFailedRepair == true then
					...
					exports['mythic_notify']:SendAlert('error', _U('car_exploding'))
					ClearPedTasksImmediately(playerPed)
					SetVehicleEngineHealth(vehicle, 0)
					Wait(4000)
					SetVehicleTimedExplosion(vehicle)
				else
					exports['mythic_notify']:SendAlert('success', _U('finished_repair'))
				end
		...
		Citizen.CreateThread(function()
			...
			if IsControlJustPressed(0, Keys["X"]) and isReparing == true then
				...
				if Config.EnableProgressBar == true then
				exports['nrrp_progbar']:startUI(300, _U('Cancelling'))
				else
				end
				exports['mythic_notify']:SendAlert('success', _U('aborted_repair'))
				isReparing = not isReparing
				CurrentAction = nil
...
AddEventHandler('tyrekit:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		local vehicle = nil
			exports['mythic_notify']:SendAlert('inform', _U('not_near_tyre'))
		else
			exports['mythic_notify']:SendAlert('inform', _U('no_vehicle_nearby'))
		end	
		...
		then
			...
			Citizen.CreateThread(function()
				...
				DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				if Config.EnableProgressBar == true then
				exports['nrrp_progbar']:startUI(Config.TyreKitTime * 1000, _U('ReparingTyre'))
				else
				end
				...
				if CurrentAction ~= nil then
					...
					exports['mythic_notify']:SendAlert('success', _U('finished_tyre_repair'))
		...
		Citizen.CreateThread(function()
			...
			if IsControlJustPressed(0, Keys["X"]) and isReparing == true then
				...
				exports['mythic_notify']:SendAlert('inform', _U('aborted_tyre_repair'))
				isReparing = not isReparing
				CurrentAction = nil
