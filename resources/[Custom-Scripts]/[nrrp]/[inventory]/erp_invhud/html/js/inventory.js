...
window.addEventListener("message", function (event) {
    if (event.data.action == "display") {
        ...
        $(".nearbyPlayerButton").click(function () {
            ...
            $.post("http://erp_invhud/GiveItem", JSON.stringify({
                player: player,
...
function closeInventory() {
    $.post("http://erp_invhud/NUIFocusOff", JSON.stringify({}));
}
...
$(document).ready(function () {
    ...
    $('#use').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.usable) {
                ...
                $.post("http://erp_invhud/UseItem", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#give').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("http://erp_invhud/GetNearPlayers", JSON.stringify({
                    item: itemData
                }));
            }
        }
    ...
    $('#drop').droppable({
        ...
        drop: function (event, ui) {
            ...
            if (itemData.canRemove) {
                disableInventory(300);
                $.post("http://erp_invhud/DropItem", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
    ...
    $('#playerInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "second") {
                ...
                $.post("http://erp_invhud/TakeFromTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
			} else if (type === "gbox" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://erp_invhud/TakeFromGBox", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://erp_invhud/TakeFromProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "motel" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://erp_invhud/TakeFromMotel", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
			} else if (type === "safe" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://erp_invhud/TakeFromSafe", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
			} else if (type === "stash" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://erp_invhud/TakeFromStash", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://erp_invhud/TakeFromPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "shop" && itemInventory === "second") {
                disableInventory(500);
                $.post("http://erp_invhud/TakeFromShop", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://erp_invhud/PutIntoTrunk", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
			} else if (type === "gbox" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://erp_invhud/PutIntoGBox", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "property" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://erp_invhud/PutIntoProperty", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "motel" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://erp_invhud/PutIntoMotel", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
			} else if (type === "safe" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://erp_invhud/PutIntoSafe", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
			} else if (type === "stash" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://erp_invhud/PutIntoStash", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "player" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://erp_invhud/PutIntoPlayer", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            } else if (type === "shop" && itemInventory === "main") {
                disableInventory(500);
                $.post("http://erp_invhud/SellToShop", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
            }
