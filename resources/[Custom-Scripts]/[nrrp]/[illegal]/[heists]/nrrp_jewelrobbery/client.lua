...
RegisterCommand('fecharloja', function(source, args, rawCommand)
	if Config.AllowPoliceStoreClose then
		...
		if GetDistanceBetweenCoords(plyloc, -631.9449, -237.8447, 38.07262, true) < 2.0 and ispolice then
			TriggerServerEvent('nrrp_jewelrobbery:closestore')
		elseif ispolice then
			exports['mythic_notify']:SendAlert('inform', 'Tens de estar perto da porta para a fechar')
		end
	end
end)
...
RegisterNetEvent('nrrp_jewelrobbery:policeclosure')
AddEventHandler('nrrp_jewelrobbery:policeclosure', function()
	policeclosed = true
	storeclosed = false
	IsAbleToRob = false
end)
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	TriggerServerEvent('nrrp_jewelrobbery:loadconfig')
end)

RegisterNetEvent('nrrp_jewelrobbery:resetcases')
AddEventHandler('nrrp_jewelrobbery:resetcases', function(list)
...
AddEventHandler('nrrp_jewelrobbery:policenotify', function()
	for i, v in pairs(Config.PoliceJobs) do
		...
		local msg = 'Assalto á Joalharia Vangélico em progresso'
		local emergency = '112'
		TriggerServerEvent('nrrp_jobchats:robberys',{
...
RegisterNetEvent('nrrp_jewelrobbery:loadconfig')
AddEventHandler('nrrp_jewelrobbery:loadconfig', function(casestatus)
...
RegisterNetEvent('nrrp_jewelrobbery:playsound')
...
AddEventHandler('nrrp_jewelrobbery:EnteredArea', function()
	for i, v in pairs(Config.CaseLocations) do
		if v.Broken then
			CreateModelSwap(v.Pos.x, v.Pos.y, v.Pos.z, 0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
		end
...
AddEventHandler('nrrp_jewelrobbery:LeftArea', function()
	for i, v in pairs(Config.CaseLocations) do
		if v.Broken then
			RemoveModelSwap(v.Pos.x, v.Pos.y, v.Pos.z, 0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
		end
...
Citizen.CreateThread( function()
	while true do 
		...
		if IsInArea and not HasAlreadyEnteredArea then
			TriggerEvent('nrrp_jewelrobbery:EnteredArea')
			shockingevent = false
		...
		if not IsInArea and HasAlreadyEnteredArea then
			TriggerEvent('nrrp_jewelrobbery:LeftArea')
			HasAlreadyEnteredArea = false
			shockingevent = false
			IsAbleToRob = false
			storeclosed = false
	...
	while true do 
		...
		while storeclosed do
			...
			if GetDistanceBetweenCoords(plyloc, -622.2496, -230.8000, 38.05705, true)  < 10.0 then
				...
				DoScreenFadeIn(1250)
				exports['mythic_notify']:SendAlert('inform', 'A joalharia está agora fechada')
			end
			Citizen.Wait(0)
			sleep = 0
			freezedoors(true)	
		...
		while policeclosed do
			...
			if GetDistanceBetweenCoords(plyloc, -622.2496, -230.8000, 38.05705, true)  < 10.0 then
				...
				SetEntityCoords(GetPlayerPed(-1), -673.1831, -227.6621, 36.11,0,0,0,true)
				DoScreenFadeIn(1250)
				exports['mythic_notify']:SendAlert('inform', 'Loja fechada para limpezas')
			end
			Citizen.Wait(0)
			sleep = 0
			freezedoors(true)	
			 if GetDistanceBetweenCoords(plyloc, -632.81, -237.9, 38.08, false) < 2.0 then
                DrawText3Ds(- 631.4819, -237.6632, 39.07612, 'Loja fechada para renovações')
            end
		...
		while IsAbleToRob and not UnAuthJob() and (CopsOnline >= Config.MinPolice) and hasgun() do
			...
			for i, v in pairs(Config.CaseLocations) do
				if GetDistanceBetweenCoords(plyloc, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.0  and not v.Broken and not IsBusy then
					sleep = 5
					DrawText3Ds(v.Pos.x, v.Pos.y, v.Pos.z + 0.5, '~g~E:~w~ para partir')
					if IsControlJustPressed(0, 38) and not IsBusy and not IsPedWalking(ply) and not IsPedRunning(ply) and not IsPedSprinting(ply) then
						...
						if breakchance <= hasweapon.chance then
							if policenotify <= Config.PoliceNotifyBroken and not HasNotified then
								TriggerServerEvent('nrrp_jewelrobbery:policenotify')
								HasNotified = true
							end
							Citizen.Wait(2100)
							TriggerServerEvent('nrrp_jewelrobbery:playsound', v.Pos.x, v.Pos.y, v.Pos.z, 'break')
							CreateModelSwap(v.Pos.x, v.Pos.y, v.Pos.z,  0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
							ClearPedTasksImmediately(ply)
							TriggerServerEvent("nrrp_jewelrobbery:setcase", i, true)	
						else
							Citizen.Wait(2100)
							TriggerServerEvent('nrrp_jewelrobbery:playsound', v.Pos.x, v.Pos.y, v.Pos.z, 'nonbreak')
							ClearPedTasksImmediately(ply)
