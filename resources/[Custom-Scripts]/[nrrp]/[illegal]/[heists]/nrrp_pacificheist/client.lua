...
function NRRP:GetInfo()
    ESX.TriggerServerCallback("nrrp_pacificheist:GetData", function(output)
        self.info = output
        self:HandleInfo()
    end)
end
function NRRP:HandleInfo()
    ...
    if PlayerData.job.name ~= "police" then
        if self.info.stage == 0 then
            Citizen.CreateThread(function()
                while true do
                    local coords = GetEntityCoords(PlayerPedId())
                        ...
                        if dst <= 6 then
                            ...
                            if dst <= 1 and IsControlJustReleased(0,38) then
                                ESX.TriggerServerCallback("nrrp_pacificheist:startevent", function(output)
                                    if output == true then
                                        NRRP.initiator = true
                                        self.info.stage = 1
                                        self.info.style = 1
                    ...
                    if NRRP.stage0break then
                        NRRP.stage0break = false
                        break
                    end
                    Citizen.Wait(1)
                end
        ...
        elseif self.info.stage == 1 then
            if NRRP.initiator then
                if self.info.style == 1 then
                    Citizen.CreateThread(function()
                        while true do
                            local coords = GetEntityCoords(PlayerPedId())

                            if not NRRP.checks.hack1 then
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("nrrp_pacificheist:checkItem", function(output)
                                            if output then
                                                NRRP.checks.hack1 = true
                                                self.currenthack = 0
                                                self:Hack()
                            ...
                            if NRRP.checks.hack1 and not NRRP.checks.hack2 then
                            ...
                            if NRRP.stage1break then
                                NRRP.stage1break = false
                                break
                            end
                            Citizen.Wait(1)
                        end
                ...
                elseif self.info.style == 2 then
                    Citizen.CreateThread(function()
                        while true do
                            ...
                            if not NRRP.searchinfo.found then
                                for i = 1, #NRRP.searchlocations, 1 do
                                    if not NRRP.searchlocations[i].status then
                                        if GetDistanceBetweenCoords(coords, NRRP.searchlocations[i].coords.x, NRRP.searchlocations[i].coords.y, NRRP.searchlocations[i].coords.z, true) <= 2.2 then
                                            DrawText3D(NRRP.searchlocations[i].coords.x, NRRP.searchlocations[i].coords.y, NRRP.searchlocations[i].coords.z, self.text.usesearch, 0.40)
                                            if IsControlJustReleased(0, 38) then
                                                Search(NRRP.searchlocations[i])
                                                Citizen.Wait(1000)
                                            end
                                        end
                                    end
                            ...
                            if NRRP.checks.id1 and not NRRP.checks.id2 then
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("nrrp_pacificheist:checkItem", function(output)
                                        if output then
                                            TriggerServerEvent("nrrp_pacificheist:removeitem", "bankid_card")
                                            NRRP.checks.id2 = true
                                            self.currentid = 2
                                            self:IdCard()
                                        elseif not output then
                                            exports["mythic_notify"]:SendAlert("error", "Não tens nenhum ID Card.")
                ...
                if self.info.style == 1 then
                    Citizen.CreateThread(function()
                        while true do
                            ...
                            if not NRRP.checks.thermal1 then
                                ...
                                if dst <= 4 then
                                    ...
                                    if dst <= 1 and IsControlJustReleased(0, 38) then
                                        ESX.TriggerServerCallback("nrrp_pacificheist:checkItem", function(output)
                                            if output then
                                                TriggerServerEvent("nrrp_pacificheist:removeitem", "thermal_charge")
                                                NRRP.checks.thermal1 = true
                                                self.currentplant = 1
                                                self:Plant()
                                            elseif not output then
                            ...
                            if NRRP.checks.thermal1 and not NRRP.checks.thermal2 then
                            ...
                            if NRRP.stage2break then
                                NRRP.stage2break = false
                                break
                            end
                            Citizen.Wait(1)
                        end
                    ...
                    if NRRP.initiator then
                        Citizen.CreateThread(function()
                            while true do
                                ...
                                if not NRRP.checks.lockpick1 then
                                    ...
                                    if dst <= 4 then
                                        ...
                                        if dst <= 1 and IsControlJustReleased(0, 38) then
                                            ESX.TriggerServerCallback("nrrp_pacificheist:checkItem", function(output)
                                                if output then
                                                    TriggerServerEvent("nrrp_pacificheist:removeitem", "lockpick")
                                                    NRRP.checks.lockpick1 = true
                                                    self.currentpick = 1
                                                    self:Lockpick()
                                                elseif not output then
                                ...
                                if not NRRP.checks.lockpick2 then
...
function NRRP:Plant()
	...
	exports["nrrp_progbar"]:startUI(4500, self.text.thermal)
    Citizen.Wait(1500)
    ...
    Citizen.Wait(4000)
	exports["nrrp_progbar"]:startUI(12000, self.text.burning)
    DeleteObject(bag)
    SetPedComponentVariation(ped, 5, 45, 0, 0)
    DetachEntity(bomba, 1, 1)
    FreezeEntityPosition(bomba, true)
    TriggerServerEvent("nrrp_pacificheist:ptfx", method)
    SetPtfxAssetNextCall("scr_ornate_heist")
    local effect = StartParticleFxLoopedAtCoord("scr_heist_ornate_thermal_burn", ptfx, 0.0, 0.0, 0.0, 1.0, false, false, false, false)

    NetworkStopSynchronisedScene(bagscene)
    NRRP.disableinput = false
    if self.currentplant == 0 then
        TriggerServerEvent("nrrp_pacificheist:fixdoor", self.doorchecks[1].h, vector3(self.doorchecks[1].x, self.doorchecks[1].y, self.doorchecks[1].z), self.doorchecks[1].he)
    elseif self.currentplant == 1 then
        TriggerServerEvent("nrrp_pacificheist:fixdoor", self.doorchecks[4].h, vector3(self.doorchecks[4].x, self.doorchecks[4].y, self.doorchecks[4].z), self.doorchecks[4].he)
    elseif self.currentplant == 2 then
        TriggerServerEvent("nrrp_pacificheist:fixdoor", self.doorchecks[5].h, vector3(self.doorchecks[5].x, self.doorchecks[5].y, self.doorchecks[5].z), self.doorchecks[5].he)
    end
    ...
    Citizen.Wait(2000)
    if self.currentplant == 0 then
        TriggerServerEvent("nrrp_pacificheist:alarm_s", 1)
    end
    DeleteObject(bomba)
    TriggerServerEvent("nrrp_pacificheist:moltgate", loc.x, loc.y, loc.z, oldmodel, newmodel)
    Citizen.Wait(9000)
    exports['mythic_notify']:SendAlert("success", self.text.melted)
    StopParticleFxLooped(effect, 0)
    if self.currentplant == 0 then
        TriggerServerEvent("nrrp_pacificheist:policeDoor", 1, false)
        self:HandleInfo()
    elseif self.currentplant == 1 then
        TriggerServerEvent("nrrp_pacificheist:policeDoor", 4, false)
    elseif self.currentplant == 2 then
        TriggerServerEvent("nrrp_pacificheist:policeDoor", 5, false)
    end
end
function NRRP:Lockpick()
    NRRP.disableinput = true
    local loc = {x, y, z, h}
	...
	exports["nrrp_progbar"]:startUI(6000, self.text.lockpick..": Etapa 1")
    Citizen.Wait(6500)
    exports['mythic_notify']:SendAlert("success", "Etapa 1 "..self.text.stage)
    SetEntityCoords(ped, loc.x, loc.y, loc.z, 1, 0, 0, 1)
    SetEntityHeading(ped, loc.h)
    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 6000, 1, 1, 0, 0, 0)
	exports["nrrp_progbar"]:startUI(6000, self.text.lockpick..": Etapa 2")
    Citizen.Wait(6500)
    ...
    if self.currentpick == 0 then
        NRRP.stage0break = true
        TriggerServerEvent("nrrp_pacificheist:policeDoor", 2, false)
        self:HandleInfo()
    elseif self.currentpick == 1 then
        TriggerServerEvent("nrrp_pacificheist:policeDoor", 4, false)
    elseif self.currentpick == 2 then
...
function NRRP:IdCard()
	...
	exports["nrrp_progbar"]:startUI(2000, NRRP.text.card)
    Citizen.Wait(1500)
    ...
    if self.currentid == 1 then
        NRRP.disableinput = false
        exports['mythic_notify']:SendAlert("success", NRRP.text.unlocked)
        TriggerServerEvent("nrrp_pacificheist:policeDoor", 3, false)
    elseif self.currentid == 2 then
        NRRP.stage1break = true
        NRRP.disableinput = false
		exports["nrrp_progbar"]:startUI(4000, NRRP.text.using)
        local count = 4
        repeat
            Citizen.Wait(1000)
            count = count - 1
        until count == 0
        exports['mythic_notify']:SendAlert("success", NRRP.text.used)
        SpawnObj()
        TriggerServerEvent("nrrp_pacificheist:openvault", 1)
        TriggerEvent("nrrp_pacificheist:vaulttimer", 2)
        TriggerServerEvent("nrrp_pacificheist:alarm_s", 1)
        self.info.stage = 2
        self.info.style = 2
        self:HandleInfo()
...
function SpawnObj()
    ...
    TriggerServerEvent("nrrp_pacificheist:startloot")
end
			    ...
			    if HasAnimEventFired(ped, GetHashKey("RELEASE_CASH_DESTROY")) then
				    if IsEntityVisible(grabobj) then
                        ...
                        if currentgrab < 4 then
                            TriggerServerEvent("nrrp_pacificheist:rewardCash")
                        elseif currentgrab == 4 then
                            TriggerServerEvent("nrrp_pacificheist:rewardGold")
                        elseif currentgrab == 5 then
                            TriggerServerEvent("nrrp_pacificheist:rewardDia")
                        end
				    end
			    end
		    end
...
function Search(location)
	...
	exports["nrrp_progbar"]:startUI(15000, NRRP.text.search)
    Citizen.Wait(15000)
    if NRRP.searchinfo.random ~= 1 then
        location.status = true
        exports['mythic_notify']:SendAlert("error", NRRP.text.nothing)
        NRRP.searchinfo.random = math.random(1, NRRP.cur - 1)
        NRRP.cur = NRRP.cur - 1
    else
        NRRP.searchinfo.found = true
        exports['mythic_notify']:SendAlert("success", NRRP.text.found)
        TriggerServerEvent("nrrp_pacificheist:giveidcard")
    end
    ClearPedTasks(PlayerPedId())
    NRRP.disableinput = false
end

function Process(ms, text) 	exports["nrrp_progbar"]:startUI(ms, text) Citizen.Wait(ms) end
...
function ShowVaultTimer() SetTextFont(0) SetTextProportional(0) SetTextScale(0.42, 0.42) SetTextDropShadow(0, 0, 0, 0,255) SetTextEdge(1, 0, 0, 0, 255) SetTextEntry("STRING") AddTextComponentString("~r~"..NRRP.vaulttime.."~w~") DrawText(0.682, 0.96) end
        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                elseif Hacking and program == 86 then
                    ...
                    if NRRP.hackmethod == 1 then
                        TriggerServerEvent("nrrp_pacificheist:policeDoor", 3, false)
                        exports['mythic_notify']:SendAlert("success", NRRP.text.hacked)
                        UsingComputer = false
                        NRRP.disableinput = false
                        NRRP.hackfinish = true
                        Ipfinished = false
                    elseif NRRP.hackmethod == 2 then
                        SpawnObj()
                        UsingComputer = false
                        Ipfinished = false
                        Process(25000, "A Hackear o sistema...")
                        TriggerEvent("nrrp_pacificheist:vaulttimer", 1)
                        TriggerServerEvent("nrrp_pacificheist:openvault", 1)
                        exports['mythic_notify']:SendAlert("success", NRRP.text.hacked)
                        NRRP.disableinput = false
                        NRRP.hackfinish = true
                ...
                elseif program == 6 then
                    TriggerServerEvent("NRRP:upVar_s", "hacking_s", false)
                    UsingComputer = false
                    NRRP.hackfinish = true
                    NRRP.disableinput = false
                    SetScaleformMovieAsNoLongerNeeded(scaleform)
...
AddEventHandler('esx:setJob', function(job)
    ...
    NRRP.stage0break = true
    NRRP.stage1break = true
    NRRP.stage2break = true
    NRRP.stage3break = true
    NRRP.stage4break = true
    NRRP.stagelootbreak = true
    NRRP.disableinput = false
    NRRP.cur = 7
    NRRP.starttimer = false
    NRRP.vaulttime = 0
    NRRP.alarmblip = nil
    NRRP.grabber = false
    NRRP.searchlocations = {
        {coords = {x = 233.40, y = 221.53, z = 110.40}, status = false},
        {coords = {x = 240.93, y = 211.12, z = 110.40}, status = false},
        {coords = {x = 246.54, y = 208.86, z = 110.40}, status = false},
        {coords = {x = 264.33, y = 212.16, z = 110.40}, status = false},
    ...
    NRRP.checks.hack1 = false
    NRRP.checks.hack2 = false
    NRRP.checks.thermal1 = false
    NRRP.checks.thermal2 = false
    NRRP.checks.id1 = false
    NRRP.checks.id2 = false
    NRRP.checks.idfound = false
    NRRP.checks.grab1 = false
    NRRP.checks.grab2 = false
    NRRP.checks.grab3 = false
    NRRP.checks.grab4 = false
    NRRP.checks.grab5 = false
    NRRP.searchinfo = {
        random = math.random(1, NRRP.cur),
        found = false
    }
    ...
    Citizen.Wait(5000)
    NRRP.hackfinish = false
    NRRP.stagelootbreak = false
    NRRP.stage0break = false
    NRRP.stage1break = false
    NRRP.stage2break = false
    NRRP.stage3break = false
    NRRP.stage4break = false
    NRRP.initiator = false
    NRRP:GetInfo()
end)
RegisterNetEvent("nrrp_pacificheist:policeDoor_c")
AddEventHandler("nrrp_pacificheist:policeDoor_c", function(doornum, status)
    PoliceDoors[doornum].locked = status
end)
RegisterNetEvent("nrrp_pacificheist:openvault_c")
AddEventHandler("nrrp_pacificheist:openvault_c", function(method)
    TriggerEvent("nrrp_pacificheist:vault", method)
    TriggerEvent("nrrp_pacificheist:vaultsound")
end)
RegisterNetEvent("nrrp_pacificheist:vault")
AddEventHandler("nrrp_pacificheist:vault", function(method)
	--local obj = ESX.Game.GetClosestObject(NRRP.vault.type, vector3(NRRP.vault.x, NRRP.vault.y, NRRP.vault.z))
	local obj = GetClosestObjectOfType(NRRP.vault.x, NRRP.vault.y, NRRP.vault.z, 2.0, GetHashKey(NRRP.vault.type), false, false, false)
    local count = 0
...
RegisterNetEvent("nrrp_pacificheist:vaultsound")
AddEventHandler("nrrp_pacificheist:vaultsound", function()
    local coords = GetEntityCoords(PlayerPedId())
    local count = 0

    if GetDistanceBetweenCoords(coords, NRRP.vault.x, NRRP.vault.y, NRRP.vault.z, true) <= 10 then
        repeat
...
RegisterNetEvent('nrrp_pacificheist:alarm')
AddEventHandler('nrrp_pacificheist:alarm', function(status)
    if status == 1 then
        PlaySound = true
...
RegisterNetEvent("nrrp_pacificheist:updatecheck_c")
AddEventHandler("nrrp_pacificheist:updatecheck_c", function(var, status)
    NRRP.checks[var] = status
end)
RegisterNetEvent("nrrp_pacificheist:startloot_c")
AddEventHandler("nrrp_pacificheist:startloot_c", function()
    NRRP.startloot = true
end)
RegisterNetEvent("nrrp_pacificheist:vaulttimer")
AddEventHandler("nrrp_pacificheist:vaulttimer", function(method)
    if method == 1 then
        exports['mythic_notify']:SendAlert("error", "Tens 180 segundos até o gás entrar na sala.")
        NRRP.starttimer = true
        NRRP.vaulttime = 180
        Citizen.CreateThread(function()
            while true do
                Citizen.Wait(1)
                if NRRP.starttimer then
                    ShowVaultTimer()
                elseif not NRRP.starttimer then
                    break
                end
            end
        end)
        repeat
            Citizen.Wait(1000)
            NRRP.vaulttime = NRRP.vaulttime - 1
        until NRRP.vaulttime == 0
        TriggerServerEvent("nrrp_pacificheist:ostimer")
        TriggerServerEvent("nrrp_pacificheist:gas")
        Citizen.CreateThread(function()
        ...
        Citizen.Wait(30000)
        NRRP.begingas = false
        TriggerServerEvent("nrrp_pacificheist:alarm_s", 2)
        ESX.TriggerServerCallback("nrrp_pacificheist:gettotalcash", function(result)
...
RegisterNetEvent("nrrp_pacificheist:gas_c")
AddEventHandler("nrrp_pacificheist:gas_c", function()
    Citizen.CreateThread(function()
        NRRP.begingas = true
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if NRRP.begingas then
                ...
                Citizen.Wait(12000)
                for i = 1, #NRRP.obj, 1 do
                    local entity = GetClosestObjectOfType(NRRP.obj[i].x, NRRP.obj[i].y, NRRP.obj[i].z, 1.0, NRRP.obj[i].h, false, false, false)

                    if DoesEntityExist(entity) then
                        DeleteEntity(entity)
                    end
                end
                for j = 1, #NRRP.emptyobjs, 1 do
                    local entity = GetClosestObjectOfType(NRRP.emptyobjs[j].x, NRRP.emptyobjs[j].y, NRRP.emptyobjs[j].z, 1.0, NRRP.emptyobjs[j].h, false, false, false)
                ...
                TriggerEvent("nrrp_pacificheist:vault", 2)
                TriggerEvent("nrrp_pacificheist:vaultsound")
                for k, v in ipairs(NRRP.doorchecks) do
                    if k == 1 or k == 4 or k == 5 then
                        TriggerEvent("nrrp_pacificheist:moltgate_c", v.x, v.y, v.z, v.h1, v.h2, 2)
                    end
                end
                NRRP.begingas = false
                if NRRP.grabber then
                    ESX.TriggerServerCallback("nrrp_pacificheist:gettotalcash", function(result)
                        text = ESX.Math.GroupDigits(result) .. "€"
                        exports["mythic_notify"]:SendAlert("success", "Roubaste "..text)
                    end)
...
RegisterNetEvent("nrrp_pacificheist:policenotify")
AddEventHandler("nrrp_pacificheist:policenotify", function(toggle)
    ...
    if player.job.name == "police" then
        if toggle == 1 then
            ...
            if not DoesBlipExist(NRRP.alarmblip) then
                NRRP.alarmblip = AddBlipForCoord(NRRP.loudstart.x, NRRP.loudstart.y, NRRP.loudstart.z)
                SetBlipSprite(NRRP.alarmblip, 161)
	            SetBlipScale(NRRP.alarmblip, 2.0)
	            SetBlipColour(NRRP.alarmblip, 1)

	            PulseBlip(NRRP.alarmblip)
            end
        elseif toggle == 2 then
            if DoesBlipExist(NRRP.alarmblip) then
                RemoveBlip(NRRP.alarmblip)
            end
        end
    end
end)
--[[RegisterNetEvent("nrrp_pacificheist:toggleDoor_c")
AddEventHandler("nrrp_pacificheist:toggleDoor_c", function(door, coord, status)
    local obj = ESX.Game.GetClosestObject(door, coord)

    FreezeEntityPosition(obj, status)
end)]]
RegisterNetEvent("nrrp_pacificheist:moltgate_c")
AddEventHandler("nrrp_pacificheist:moltgate_c", function(x, y, z, oldmodel, newmodel, method)
    if method == 2 then
...
RegisterNetEvent("nrrp_pacificheist:fixdoor_c")
AddEventHandler("nrrp_pacificheist:fixdoor_c", function(hash, coords, heading)
...
RegisterNetEvent("nrrp_pacificheist:ptfx_c")
AddEventHandler("nrrp_pacificheist:ptfx_c", function(method)
    local ptfx
...
RegisterNetEvent("nrrp_pacificheist:reset")
AddEventHandler("nrrp_pacificheist:reset", function()
    NRRP.stage1break = true
    NRRP.stage2break = true
    NRRP.stage4break = true
    NRRP.stagelootbreak = false
    NRRP.disableinput = false
    NRRP.info = {stage = 0, style = nil, locked = false}
    NRRP.cur = 7
    NRRP.starttimer = false
    NRRP.vaulttime = 0
    NRRP.alarmblip = nil
...
Citizen.CreateThread(function()
    if NRRP.enablextras then
        while true do
            if NRRP.startloot then
                local coords = GetEntityCoords(PlayerPedId())

                if not NRRP.checks.grab1 then
                    local dst1 = GetDistanceBetweenCoords(coords, NRRP.cash1.x, NRRP.cash1.y, NRRP.cash1.z, true)

                    if dst1 <= 4 then
                        DrawText3D(NRRP.cash1.x, NRRP.cash1.y, NRRP.cash1.z, NRRP.text.lootcash, 0.40)
                        if dst1 < 1 and IsControlJustReleased(0, 38) then
                            NRRP.checks.grab1 = true
                            TriggerServerEvent("nrrp_pacificheist:updatecheck", "grab1", true)
                            Loot(1)
                        end
                    end
                end
                if not NRRP.checks.grab2 then
                    local dst2 = GetDistanceBetweenCoords(coords, NRRP.cash2.x, NRRP.cash2.y, NRRP.cash2.z, true)

                    if dst2 <= 4 then
                        DrawText3D(NRRP.cash2.x, NRRP.cash2.y, NRRP.cash2.z, NRRP.text.lootcash, 0.40)
                        if dst2 < 1 and IsControlJustReleased(0, 38) then
                            NRRP.checks.grab2 = true
                            TriggerServerEvent("nrrp_pacificheist:updatecheck", "grab2", true)
                            Loot(2)
                        end
                    end
                end
                if not NRRP.checks.grab3 then
                    local dst3 = GetDistanceBetweenCoords(coords, NRRP.cash3.x, NRRP.cash3.y, NRRP.cash3.z, true)

                    if dst3 <= 4 then
                        DrawText3D(NRRP.cash3.x, NRRP.cash3.y, NRRP.cash3.z, NRRP.text.lootcash, 0.40)
                        if dst3 < 1 and IsControlJustReleased(0, 38) then
                            NRRP.checks.grab3 = true
                            TriggerServerEvent("nrrp_pacificheist:updatecheck", "grab3", true)
                            Loot(3)
                        end
                    end
                end
                if not NRRP.checks.grab4 then
                    local dst4 = GetDistanceBetweenCoords(coords, NRRP.gold.x, NRRP.gold.y, NRRP.gold.z, true)

                    if dst4 <= 4 then
                        DrawText3D(NRRP.gold.x, NRRP.gold.y, NRRP.gold.z, NRRP.text.lootgold, 0.40)
                        if dst4 < 1 and IsControlJustReleased(0, 38) then
                            NRRP.checks.grab4 = true
                            TriggerServerEvent("nrrp_pacificheist:updatecheck", "grab4", true)
                            Loot(4)
                        end
                    end
                end
                if not NRRP.checks.grab5 then
                    local dst5 = GetDistanceBetweenCoords(coords, NRRP.dia.x, NRRP.dia.y, NRRP.dia.z, true)

                    if dst5 <= 4 then
                        DrawText3D(NRRP.dia.x, NRRP.dia.y, NRRP.dia.z, NRRP.text.lootdia, 0.40)
                        if dst5 < 1 and IsControlJustReleased(0, 38) then
                            NRRP.checks.grab5 = true
                            TriggerServerEvent("nrrp_pacificheist:updatecheck", "grab5", true)
                            Loot(5)
                        end
                    end
                end
                if NRRP.stagelootbreak then
                    return
                end
            end
            Citizen.Wait(1)
        end
    elseif not NRRP.enablextras then
        while true do
            if NRRP.startloot then
                local coords = GetEntityCoords(PlayerPedId())
...
Citizen.CreateThread(function()
    ...
    ESX.TriggerServerCallback("nrrp_pacificheist:GetDoors", function(result)
        PoliceDoors = result
    end)
            ...
            if dst <= 50 then
                ...
                if PlayerData.job.name == "police" then
                    if dst <= 1.5 then
                        ...
                        if IsControlJustReleased(0, 38)  then
                            TriggerServerEvent("nrrp_pacificheist:policeDoor", k, state)
                        end
                    end
                end
            end
...
Citizen.CreateThread(function() while ESX == nil do TriggerEvent('esx:getSharedObject', function(obj) ESX = obj end) Citizen.Wait(1) end Citizen.Wait(10000) NRRP:GetInfo() end)
