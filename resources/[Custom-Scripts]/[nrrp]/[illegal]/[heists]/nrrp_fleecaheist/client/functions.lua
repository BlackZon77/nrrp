GlobalFunction = function(a, b)
    ...
    TriggerServerEvent("nrrp_fleecaheist:globalEvent", c)
end

TryHackingDevice = function(d)
    ESX.TriggerServerCallback("nrrp_fleecaheist:fetchCops", function(e)
		if e then
			StartHackingDevice(d)
		else
			ESX.ShowNotification("Polícia insuficiente.")
		end
	end)
end
...
StartHackingDevice = function(d)
    Citizen.CreateThread(function()
		...
		if #Config.ItemNeeded > 0 then
			if not HasItem() then
				return exports['mythic_notify']:SendAlert('error', 'Não tens nenhum ' .. Config.ItemNeeded)
			end
		end
		...
		if g ~= -1 and h <= 3.0 then
			if
			 ...
			 then
				return exports['mythic_notify']:SendAlert('error', 'Outra pessoa está a tentar desencriptar a fechadura')
			end
		end
		...
		Citizen.Wait(2500)
		-- StartComputer()
		TriggerEvent("nrrp_fingerprint:Start", 4, 6, 2, function(outcome, reason)
...
GrabCash = function(s)
			...
			while GetGameTimer() - B < 37000 do
				...
				if HasAnimEventFired(w, GetHashKey("RELEASE_CASH_DESTROY")) then
					if IsEntityVisible(A) then
						SetEntityVisible(A, false, false)
						TriggerServerEvent("nrrp_fleecaheist:receiveCash")
					end
				end
			end
    ...
    if IsEntityPlayingAnim(v, "anim@heists@ornate_bank@grab_cash", "cart_cash_dissapear", 3) then
        return ESX.ShowNotification("Alguem já está a pegar.")
    end
					...
					if NetworkDoesEntityExistWithNetworkId(s["net"]) then
						...
						if G <= 1.5 then
							if Config.BlackMoney then
								ESX.ShowHelpNotification("~INPUT_DETONATE~ Pegar ~r~dinheiro ~s~do ~y~carrinho")
							else
								ESX.ShowHelpNotification("~INPUT_DETONATE~ Pegar ~g~dinheiro ~s~do ~y~carrinho")
							end
...
ToggleBag = function(K)
    TriggerEvent("skinchanger:getSkin", function(L)
		if L.sex == 0 then
