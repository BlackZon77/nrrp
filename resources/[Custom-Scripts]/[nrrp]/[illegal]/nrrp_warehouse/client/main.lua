...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    ...
    ESX.TriggerServerCallback("nrrp_warehouse:getWH", function(list)
        if list == nil then
            shouldSave = true
        else
            Warehouse = list
    ...
    for i = 1, #Warehouse, 1 do -- this should ensure that objects are created for everyone (not sure tho)
        ...
        TriggerServerEvent("nrrp_warehouse:updateWH", Warehouse)
    end
end)

RegisterNetEvent("nrrp_warehouse:upVar")
AddEventHandler("nrrp_warehouse:upVar", function(var, status)
    if var == "locked" then
        locked = status
    end
...
RegisterNetEvent("nrrp_warehouse:start")
AddEventHandler("nrrp_warehouse:start", function(action)
    ...
    Starttimer = true
    Busy = true
    TriggerServerEvent("nrrp_warehouse:upVar_s", "busy", true)
    exports['mythic_notify']:SendAlert("success", _U("pickup_s"))
    Citizen.Wait(500)
    exports['mythic_notify']:SendAlert("success", _U("15min"))
end)

RegisterNetEvent("nrrp_warehouse:sell")
AddEventHandler("nrrp_warehouse:sell", function(count, amount)
    Timer = Config.selltime
    GetSellInfo(count, amount)
    TriggerEvent("nrrp_warehouse:startSell")
    Citizen.Wait(1000)
    Sellstart = true
    Starttimer = true
    Busy = true
    ...
    NPCstart = true
    Citizen.Wait(1000)
    TriggerEvent("nrrp_warehouse:delobj")
end)

RegisterNetEvent("nrrp_warehouse:itemEkle")
AddEventHandler("nrrp_warehouse:itemEkle", function(currentslot)
...
RegisterNetEvent("nrrp_warehouse:delobj")
AddEventHandler("nrrp_warehouse:delobj", function()
    for i = 1, #Warehouse, 1 do
        if Warehouse[i].empty == true then
            ...
            local obj = ESX.Game.GetClosestObject((Warehouse[i].obj), vector3(Warehouse[i].x, Warehouse[i].y, Warehouse[i].z))
    ...
    while true do
        ...
        while Sellstart or pickup or whstart or Delstart do
            ...
            if IsEntityDead(ped) then -- This works fine
                ...
                Starttimer = false
                Busy = false
                TriggerServerEvent("nrrp_warehouse:upVar_s", "busy", false)
                RemoveBlip(currentblip)
                RemoveBlip(Enemyblip)
                RemoveBlip(vehblip)
                currentblip = nil
        ...
        if whstart then
            ...
            if dst2 <= 2.5 then
                ...
                if vehname == needcar and vehicle then
                    ...
                    Citizen.Wait(1000)
                    DoScreenFadeIn(1000)
                    TriggerEvent("nrrp_warehouse:itemEkle", currentslot)
                    header = _U("success")
                    text = _U("delivered")
                    Citizen.Wait(200)
                    draw = true
        ...
        if Sellstart then
            ...
            if vehname == needcar then
                if (GetDistanceBetweenCoords(GetEntityCoords(player), Currentpos, true) <= 2.5) then
                    ...
                    Citizen.Wait(1000)
                    DoScreenFadeIn(1000)
                    TriggerServerEvent("nrrp_warehouse:sellGoods", Reward)
                    header = _U("success")
                    text = _U("sold")
                    Citizen.Wait(200)
                    draw = true
    ...
    while true do
        ...
        if dstCheck <= 2 then
            if IsControlJustReleased(0, 38) then
                ...
                Citizen.Wait(500)
                TriggerServerEvent("nrrp_warehouse:checkKey", 1)
                Citizen.Wait(200)
                DoScreenFadeIn(1000)
                if shouldlock then
                    TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 2000, 1, 1, 0, 0, 0)
                    exports['nrrp_progbar']:startUI(2000, _U("locking"))
                else
    ...
    while true do
        ...
        if dst2 <= 1.5 then
            if IsControlJustReleased(0, 38) and locked then
                TriggerServerEvent("nrrp_warehouse:checkKey", 2)
            elseif IsControlJustReleased(0, 38) and not locked then
                TriggerEvent("nrrp_warehouse:tpWH", true, false)
            end
        end
    end
end)
    ...
    while true do
        ...
        if IsControlJustReleased(0, 38) and dst <= 1.5 then
            if not Delstart or not Sellstart then
                TriggerServerEvent("nrrp_warehouse:checkKey", 3)
            else
...
RegisterNetEvent("nrrp_warehouse:startSell")
AddEventHandler("nrrp_warehouse:startSell", function()
    local ped = PlayerPedId()
    ...
    drawmarker = true
    TriggerServerEvent("nrrp_warehouse:upVar_s", "locked", true)
end)

RegisterNetEvent("nrrp_warehouse:openLaptop")
AddEventHandler("nrrp_warehouse:openLaptop", function()
    if not Busy then
        local ped = PlayerPedId()
...
RegisterNetEvent("nrrp_warehouse:tpWH") --TP to warehouse
AddEventHandler("nrrp_warehouse:tpWH", function(output, method)
    if output then
        local ped = PlayerPedId()
        local animDict = "mp_arresting"
        ...
        TaskPlayAnim(ped, animDict, "a_uncuff", 8.0, 8.0, 2000, 1, 1, 0, 0, 0)
        if method then
            exports['nrrp_progbar']:startUI(2000, _U("unlocking"))
        elseif not method then
            exports['nrrp_progbar']:startUI(2000, _U("entering"))
        end
        Citizen.Wait(1000)
        DoScreenFadeOut(1000)
        Citizen.Wait(1000)
