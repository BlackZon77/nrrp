ESX = nil
local nocontrols, allowedchop, cooldown, chopstarted = false, true, false, false
local station = nil
...
function away()
	...
	exports['mythic_notify']:SendAlert('success', 'Vai te embora')
end
				...
				if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), ChopCarLocation[station].Chop, true ) < 1.2 then 
					...
					if(IsControlJustPressed(0, 38)) then
						...
						if cooldown and not chopstarted then
							exports['mythic_notify']:SendAlert('inform', 'Espera uns minutos')
						end
					end
				end
			else
...
function EndChopping()
	TriggerServerEvent('nrrp_chopshop:success', math.random(30,80))
	local vehchopping = GetClosestVehicle(ChopCarLocation[station].Chop, 4.001, 0, 70)
	chopstarted = false
	DeleteEntity(vehchopping)
	exports['mythic_notify']:SendAlert('success', 'O carro foi desmantelado por completo')
	for i=1, #chopdoors, 1 do
		chopdoors[i].chopdoor = false
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					if chopdoors[i].doorgone then
						TriggerServerEvent('nrrp_chopshop:success')
						chopdoors[i].doorgone = false
						chopdoors[i].doordelivered = true
						nocontrols = false
						exports['mythic_notify']:SendAlert('success', 'Peças vendidos com sucesso')
