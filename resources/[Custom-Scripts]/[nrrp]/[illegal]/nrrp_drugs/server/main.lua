...
RegisterServerEvent("nrrp_drugs:startHackTimer")
AddEventHandler("nrrp_drugs:startHackTimer",function(source)
	table.insert(PlayerHackTimer,{started = GetPlayerIdentifier(source), time = 7200000}) -- cooldown timer for using USB stick
end)

RegisterServerEvent("nrrp_drugs:startDrugsTimer") 
AddEventHandler("nrrp_drugs:startDrugsTimer",function(source)
	table.insert(PlayerDrugsTimer,{startedDrugs = GetPlayerIdentifier(source), timeDrugs = 15000}) -- do not touch this
end)

RegisterServerEvent("nrrp_drugs:startConvertTimer")
AddEventHandler("nrrp_drugs:startConvertTimer",function(source)
...
RegisterServerEvent("nrrp_drugs:reward")
AddEventHandler("nrrp_drugs:reward",function(amount,typed)
	local xPlayer = ESX.GetPlayerFromId(source)
	xPlayer.addInventoryItem(typed.."brick",math.ceil(amount))
end)

RegisterServerEvent("nrrp_drugs:syncMissionData")
AddEventHandler("nrrp_drugs:syncMissionData",function(data)
	TriggerClientEvent("nrrp_drugs:syncMissionData",-1,data)
end)

ESX.RegisterUsableItem('methburn', function(source)
	if not CheckedStarted(GetPlayerIdentifier(source)) then
		TriggerEvent("nrrp_drugs:startHackTimer",source)
		TriggerClientEvent("nrrp_drugs:UsableItem",source)
		Citizen.Wait(8000)
		TriggerClientEvent("nrrp_drugs:HackingMiniGame",source)
			
		ESX.RegisterServerCallback("nrrp_drugs:StartMissionNow",function(source,cb)
			local _source = source
			local xPlayer = ESX.GetPlayerFromId(_source)
			cb()
			TriggerClientEvent("nrrp_drugs:startMission",source,0,"meth")
		end)
...
ESX.RegisterUsableItem('cokeburn', function(source)
	if not CheckedStarted(GetPlayerIdentifier(source)) then
		...
		ESX.RegisterServerCallback("nrrp_drugs:StartMissionNow",function(source,cb)
			...
			local xPlayer = ESX.GetPlayerFromId(_source)
			cb()
			TriggerClientEvent("nrrp_drugs:startMission",source,0,"coke")
		end)
...
ESX.RegisterUsableItem('weedburn', function(source)
	if not CheckedStarted(GetPlayerIdentifier(source)) then
		...
		ESX.RegisterServerCallback("nrrp_drugs:StartMissionNow",function(source,cb)
			...
			local xPlayer = ESX.GetPlayerFromId(_source)
			cb()
			TriggerClientEvent("nrrp_drugs:startMission",source,0,"weed")
		end)
...
ESX.RegisterUsableItem('meth1g', function(source)
	...
	TriggerEvent("nrrp_drugs:startDrugsTimer",source)
...
ESX.RegisterUsableItem('cokebrick', function(source)
	...
	if xPlayer.getInventoryItem("coke10g").count <= 40 or (not scale and xPlayer.getInventoryItem("coke10g").count <= 44) then
		if not CheckedStartedConvert(GetPlayerIdentifier(source)) then
			TriggerEvent("nrrp_drugs:startConvertTimer",source)
...
RegisterServerEvent("nrrp_drugs:sellDrugs")
AddEventHandler("nrrp_drugs:sellDrugs", function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent("nrrp_drugs:canSellDrugs")
AddEventHandler("nrrp_drugs:canSellDrugs", function()
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer ~= nil then
		...
		TriggerClientEvent("nrrp_drugs:canSellDrugs",source,soldAmount)
	end
end)
