			...
			if distance < 2 and CanSellToPed(ped) and canSellDrugs then
				if canSellDrugs and not InVehicle then
					...
					if IsControlJustPressed(1,74) then
						...
						exports["nrrp_progbar"]:startUI(5000, "A vender drogas...")
						Citizen.Wait(5000)
						if chance == 1 then
							...
							TriggerServerEvent("nrrp_drugs:sellDrugs")
						else
...
Citizen.CreateThread(function()
	while true do
		TriggerServerEvent("nrrp_drugs:canSellDrugs")
		Citizen.Wait(10000)
	end
end)
...
RegisterNetEvent("nrrp_drugs:canSellDrugs")
AddEventHandler("nrrp_drugs:canSellDrugs", function(soldAmount)
