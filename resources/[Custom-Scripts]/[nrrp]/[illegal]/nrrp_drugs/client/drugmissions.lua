...
RegisterNetEvent("nrrp_drugs:startMission")
AddEventHandler("nrrp_drugs:startMission",function(spot,type)
	...
	if numy == 100 then
		...
		CurrentEventNum = num
		TriggerEvent("nrrp_drugs:startTheEvent",num,type)
		exports['mythic_notify']:SendAlert('inform', 'Segue a localização no teu mapa e rouba '..type)
	end
	
end)

RegisterNetEvent("nrrp_drugs:startTheEvent")
AddEventHandler("nrrp_drugs:startTheEvent",function(num,typey)
	...
	TriggerServerEvent("nrrp_drugs:syncMissionData",Config.MissionPosition)
	local taken = false
	...
	while not taken and not StopMission do
		...
		if GetDistanceBetweenCoords(loc.Location, GetEntityCoords(GetPlayerPed(-1))) < 2.5 then
			...
			if IsControlJustPressed(1,38) then					
					exports["nrrp_progbar"]:startUI(15000, "A roubar drogas...")
					Citizen.Wait(15000)
				if GetDistanceBetweenCoords(loc.Location, GetEntityCoords(GetPlayerPed(-1))) < 2.5 then
					exports['mythic_notify']:SendAlert('sucess', 'Missão Completa: Tu roubaste as drogas com sucesso')
					TriggerServerEvent("nrrp_drugs:reward",(Config.Reward[typed]),typed)
					RemoveBlip(blip)
