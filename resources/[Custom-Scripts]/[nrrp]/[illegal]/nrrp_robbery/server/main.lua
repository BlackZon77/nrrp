...
RegisterServerEvent('nrrp_robbery:tooFar')
AddEventHandler('nrrp_robbery:tooFar', function(currentStore)
	...
	for i=1, #xPlayers, 1 do
		...
		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('nrrp_robbery:killBlip', xPlayers[i])
		end
	end

	if robbers[_source] then
		TriggerClientEvent('nrrp_robbery:tooFar', _source)
		robbers[_source] = nil
		TriggerClientEvent('esx:showNotification', _source, _U('robbery_cancelled_at', Stores[currentStore].nameOfStore))
	end
end)

RegisterServerEvent('nrrp_robbery:robberyStarted')
AddEventHandler('nrrp_robbery:robberyStarted', function(currentStore)
	local _source  = source
			...
			if cops >= Config.PoliceNumberRequired then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
						...
						TriggerClientEvent('nrrp_robbery:setBlip', xPlayers[i], Stores[currentStore].position)
					end
				end
				...
				TriggerClientEvent('nrrp_robbery:currentlyRobbing', _source, currentStore)
				TriggerClientEvent('nrrp_robbery:startTimer', _source)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						rob = false
						if xPlayer then
							TriggerClientEvent('nrrp_robbery:robberyComplete', _source, store.reward)
