...
RegisterNetEvent('nrrp_robbery:currentlyRobbing')
AddEventHandler('nrrp_robbery:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('nrrp_robbery:killBlip')
AddEventHandler('nrrp_robbery:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('nrrp_robbery:setBlip')
AddEventHandler('nrrp_robbery:setBlip', function(position)
...
RegisterNetEvent('nrrp_robbery:tooFar')
AddEventHandler('nrrp_robbery:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('nrrp_robbery:robberyComplete')
AddEventHandler('nrrp_robbery:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('nrrp_robbery:startTimer')
AddEventHandler('nrrp_robbery:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, 38) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('nrrp_robbery:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('nrrp_robbery:tooFar', store)
			end
		end
	end
end)
