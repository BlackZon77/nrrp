...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent("nrrp-hud:server:requestTable")
end)
...
Citizen.CreateThread(function() 
    while true do
        Citizen.Wait(1000)
        if IsPedInAnyVehicle(PlayerPedId()) and NRRPHud then
            DisplayRadar(true)
            SendNUIMessage({
                action = "car",
                show = true,
...
Citizen.CreateThread(function()
	while true do
        ...
        if DoesEntityExist(veh) then
            ...
            TriggerEvent("nrrp-hud:client:UpdateDrivingMeters", true, math.floor(vehiclesKHM[plate]))
        end
        ...
        if IsControlJustReleased(0, 29) and IsPedInAnyVehicle(PlayerPedId()) then
            seatbeltOn = not seatbeltOn
             if not seatbeltOn then
                ...
                TriggerEvent("notification",'Sinto de segurança retirado',2)
			else
				TriggerEvent("seatbelt:client:ToggleSeatbelt",true)
				TriggerEvent("notification",'Sinto de segurança posto',1)
            end
		end
	end
end)
	...
	while true do
        ...
        if DoesEntityExist(veh) then
            ...
            if vehiclesKHM[plate] and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
                TriggerServerEvent("nrrp-hud:server:vehiclesKHM", plate, vehiclesKHM[plate])
            end
        end
	end
end)

RegisterNetEvent("nrrp-hud:client:vehiclesKHM")
AddEventHandler("nrrp-hud:client:vehiclesKHM", function(plate,kmh)
    vehiclesKHM[plate] = kmh
end)

RegisterNetEvent("nrrp-hud:client:vehiclesKHMTable")
AddEventHandler("nrrp-hud:client:vehiclesKHMTable", function(table)
    vehiclesKHM = table
end)
...
RegisterNetEvent('nrrp-hud:client:UpdateDrivingMeters')
AddEventHandler('nrrp-hud:client:UpdateDrivingMeters', function(toggle, amount)
    SendNUIMessage({
