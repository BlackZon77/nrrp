...
Citizen.CreateThread(function()
    ...
    if Config.IKEABlip['Enabled'] then
        ...
        BeginTextCommandSetBlipName("IKEA")
        AddTextEntry('IKEA', Config.IKEABlip['Name'])
        ...
        if OwnedHouse.houseId == k then
            ...
            if not AvailableHouses[k] then
                if Config.AddHouseBlips then
                    CreateBlip(v['door'], 374, 0, 0.3, Strings['Available_House'])
                end
            else
                if Config.AddBoughtHouses then
                    CreateBlip(v['door'], 374, 2, 0.3, Strings['Player_House'])
                end
            end
        end
    end
...
CreateBlip = function(coords, sprite, colour, scale, text)
    ...
    SetBlipAsShortRange(blip, 38)
    BeginTextCommandSetBlipName("CASASP")
    AddTextComponentString(text)
    AddTextEntry('CASASP', text)
    EndTextCommandSetBlipName(blip)
