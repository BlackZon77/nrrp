$(document).ready(function () {
	...
	document.onkeyup = function (data) {
		if (data.which == 27 ) {
			$.post('http://nrrp_apreendidos/escape', JSON.stringify({}));
		}
	};
	...
	function setupImpoundForm(data) {
		...
		$('#reason').attr("placeholder", `Insere um descrição com um mínimo de ${rules.minReasonLength} palavras ou mais`)
	}
	...
	$('#impound').click(function (event) {
		...
		if(validateImpoundForm()) {
			$.post('http://nrrp_apreendidos/impound', JSON.stringify({
				plate: $('#plate').text(),
	...
	function validateImpoundForm () {
		...
		if(String(weeks).length < 1 || parseInt(weeks) < 0 || parseInt(weeks) > rules.maxWeeks || String(days).length < 1 || parseInt(days) < reason.minDays || parseInt(days) > rules.maxDays || String(hours).length < 1 || parseInt(hours) < 0 || parseInt(hours) > rules.maxHours) {
			errors.append(`<small>&#9679; Semanas tem de ser 0 ou menos de ${rules.maxWeeks}, dias tem de ser 0 ou menos de ${rules.maxDays} e horas tem de ser 0 ou menos de ${rules.maxHours}.</small>`);
			success = false;
		}

		if(fee.isNaN || String(fee).length < 1 || parseInt(fee) < rules.minFee || parseInt(fee) > rules.maxFee) {
			errors.append(`<small>&#9679; Multa não pode ser menos  de ${rules.minFee} ou mais de ${rules.maxFee}</small>`);
			success = false;
		}

		if(reason.length < rules.minReasonLength || reason.length > 10000) {
			errors.append(`<small>&#9679; Razão do apreendimento tem de ter um mínimo de ${rules.minReasonLength} palavras.</small>`);
			success = false;
		}
	...
	$('table').on('click', '.pay', function () {
		...
		$.post('http://nrrp_apreendidos/unimpound', JSON.stringify(plate));
	});
	...
	$('#cancel, #exit').click(function (event) {
		$.post('http://nrrp_apreendidos/escape', null);
	});
