...
function NRRPUtils:DrawTextTemplate(text,x,y,font,scale1,scale2,colour1,colour2,colour3,colour4,wrap1,wrap2,centre,outline,dropshadow1,dropshadow2,dropshadow3,dropshadow4,dropshadow5,edge1,edge2,edge3,edge4,edge5)
    return {
      text         =                    "",
      x            =                    -1,
      y            =                    -1,
...
function NRRPUtils.PointOnSphere(alt,azu,radius,orgX,orgY,orgZ)
...
function NRRPUtils.ClampCircle(x,y,radius)
...
function NRRPUtils.GetHashKey(strToHash)
...
GetHashKey     = NRRPUtils.GetHashKey

function NRRPUtils:FindNearestZone(position, table)
    ...
    if zone.Positions then 
      for act,pos in pairs(zone.Positions) do
        if type(pos) == 'vector3' then
          local curDist = NRRPUtils:GetVecDist(position, pos)
...
function NRRPUtils:GetVecDist(v1,v2)
  if not v1 or not v2 or not v1.x or not v2.x then return 0; end
  return math.sqrt(  ( (v1.x or 0) - (v2.x or 0) )*(  (v1.x or 0) - (v2.x or 0) )+( (v1.y or 0) - (v2.y or 0) )*( (v1.y or 0) - (v2.y or 0) )+( (v1.z or 0) - (v2.z or 0) )*( (v1.z or 0) - (v2.z or 0) )  )
end

function NRRPUtils.GetCoordsInFrontOfCam(...)
    ...
    local distanceMod = Distance((coords-GetEntityCoords(PlayNRRPedId(),false) or vector3(0,0,0)))
    local retTable    = {}
...
function NRRPUtils.RotationToDirection(rot)
...
function NRRPUtils.LoadModel(model, wait)
  local hk = NRRPUtils.GetHashKey(model)
  if wait then
...
function NRRPUtils.ReleaseModel(model)
  local hk = NRRPUtils.GetHashKey(model)
...
function NRRPUtils:LoadModelTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = NRRPUtils.GetHashKey(v)
      while not HasModelLoaded(hk) do
        RequestModel(hk)
        Citizen.Wait(0)
      end
...
function NRRPUtils:ReleaseModelTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = NRRPUtils.GetHashKey(v)
...
function NRRPUtils.LoadWeaponTable(table)  
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = NRRPUtils.GetHashKey(v)
...
function NRRPUtils.ReleaseWeaponTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
      local hk = NRRPUtils.GetHashKey(v)
...
function NRRPUtils.LoadAnimTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
...
function NRRPUtils.ReleaseAnimTable(table)
  if type(table) ~= 'table' then return false; end
  for k,v in pairs(table) do
    if type(v) == 'string' then
...
function NRRPUtils:LoadAnimDict(dict)
...
function NRRPUtils:ReleaseAnimDict(dict)
  if type(dict) ~= 'string' then return false; end
...
function NRRPUtils.NetworkControlEntity(ent)
...
function NRRPUtils.NetworkControlDoor(obj)
